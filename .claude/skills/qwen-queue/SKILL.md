...
| literal `<tool_call>` as the whole summary | NInfer parses Qwen3-Coder XML correctly in normal modes but returns raw partial XML when truncated by token cap, with `finish_reason: length` and `tool_calls: null` | add shim guard to detect and fail truncated tool calls; reduce job size if it keeps hitting token limit |
