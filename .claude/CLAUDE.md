...
MCP tools (via Context Forge) and `kubectl` are both available for cluster reads. Use `ToolSearch` with `+kubernetes`, `+argocd`, or `+signoz` to load MCP tools. Tool names below are shortened — actual IDs have the `mcp__claude_ai_homelab__` prefix (e.g., `mcp__claude_ai_homelab__kubernetes-mcp-resources-list`). If a session only has the monolith MCP server registered (no kubernetes/argocd/signoz tools), fall back to `kubectl` directly rather than burning turns on `ToolSearch`.
