...
- **Sonnet 4.6 for the implementation bulk**: well-specified, locally-verifiable tasks (boilerplate, config plumbing, mechanical edits, clearly-spec'd functions). Sonnet is most of the quality at much faster turns, and this repo's dense edit-time hooks plus local renders (`format`, `helm template`, type-check) catch most misses before CI, so its speed is close to a pure wall-time win. Dispatch Sonnet implementers via the per-call `model` parameter.
- **Opus reviewers are the safety net** that makes fast Sonnet implementers safe: an Opus reviewer reading the diff before the end-of-plan CI run catches subtle misses cheaply (code-reading review, no CI cost). Sonnet hands, Opus eyes. Do not downgrade the reviewer.
...
| **Deploy / rollout**       | We do NOT use ArgoCD Image Updater. Releases roll out by building dual-arch OCI images in CI and bumping the chart version: CI rebuilds the image with a build-time-pinned tag, and a new chart version (Chart.yaml + `deploy/application.yaml` `targetRevision`, kept in sync by `chart-version-bot`) is what ArgoCD pulls from OCI and syncs. Legacy `imageupdater.yaml` / `ImageUpdater` CRs may still exist in some `projects/*/deploy/` dirs but are not the operative path (slated for removal); do not rely on them.                                                                                                             |
...
- **Bumping `Chart.yaml` without `application.yaml`** — use `bazel/tools/git/bump-chart.sh`, which updates both together; a mismatch means ArgoCD keeps deploying the old chart version with stale image digests (a sync hook also enforces the two match)
