# STPA Control Analysis: firecracker @ 50f07cb2f
...
- **Note:** Caller auth is now enforced in-app (TokenReview allow-list, set in deploy values) plus a Linkerd AuthorizationPolicy as defence in depth; the residual exposure is fail-open if FC_INVOKE_ALLOWED_CALLERS is unset, and TokenReview validates the API server default audiences rather than a dedicated fc-invoke audience.
</details>
...
flowchart TD
  ...
  subgraph entry["entry"]
    auth["Caller auth middleware (TokenReview + allow-list)"]
  ...
  auth -- "auth.review" --> ingress
  caller -- "caller.invoke" --> auth
...
| `unauthenticated-invoke` | logical | With caller auth misconfigured off (allow-list env unset) the middleware is skipped and any in-cluster client invokes a workload, driving sidecar-swapped credential privileges | L.unauthorized-access, L.secret-exposure | built |
...
|----|----|----|----|----|----|
| `agent.clone` | logical | Clone workspace from mirror | `agent-handler` → `git-mirror` | built | projects/firecracker/goosecracker/guest-init/internal/handler/handler.go:148 |
| `agent.egress` | logical | Outbound goose/tool calls captured by funnel | `agent-handler` → `egress-funnel` | built | projects/firecracker/goosecracker/guest-init/cmd/main.go:300 |
| `agent.record` | logical | Push scratch ref refs/agents/{session} | `agent-handler` → `git-mirror` | built | projects/firecracker/substrate/shim/capabilities/git.go:157 |
| `auth.review` | logical | Authenticate caller token (TokenReview) + allow-list before admit | `auth` → `ingress` | built | projects/firecracker/substrate/cluster/auth/auth.go:86 |
| `builder.bake` | physical | Crane-pull guest image, bake ext4 rootfs | `rootfs-builder` → `nvme` | built | projects/firecracker/substrate/chart/templates/deployment.yaml:76 |
| `caller.invoke` | logical | POST /invoke/{workload}[/{session}] | `caller` → `auth` | built | projects/firecracker/substrate/invoke/cmd/main.go:154 |
...
| `op.sync-secret` | physical | Deliver real secret value to sidecar env | `op-secrets` → `egress-sidecar` | built | projects/firecracker/substrate/deploy/values.yaml:101 |
...
|----|----|----|----|----|----|----|----|
| `agent.clone.not-providing` | logical | `agent.clone` | not-providing | a mirror clone failure is swallowed and goose runs on an empty workspace, so results look valid but were produced without the source | medium | empty-workspace-run | projects/firecracker/goosecracker/guest-init/internal/handler/handler.go:149 |
| `auth.review.not-providing` | logical | `auth.review` | not-providing | when FC_INVOKE_ALLOWED_CALLERS is unset the auth middleware is not installed at all, so every /invoke is admitted unauthenticated; a dropped env var silently reverts to open invoke with only the Linkerd policy as backstop | high | unauthenticated-invoke | projects/firecracker/substrate/invoke/cmd/main.go:156 |
...
| `proxy.egress-external.providing` | logical | `proxy.egress-external` | providing | external allow-by-default lets a prompt-injected agent exfiltrate the cloned private workspace to any internet host | high | workspace-exfil | projects/firecracker/substrate/deploy/values.yaml:65 |
...
- **crafted session id pushing to refs/heads**: rejected by the pre-receive hook (refs/agents/** only) plus sanitizeRefComponent (git.go:212)
...
- **undecodable or corrupt session db on resume**: handler falls back to a cold run with resume=false (handler.go:159)
...
- TokenReview validates the API server default audiences, not a dedicated fc-invoke audience (reviewer.go:44); should a projected audience-scoped caller token be required so a monolith SA token minted for another audience cannot authenticate to /invoke?
