...
- **Note:** Caller auth is now enforced in-app (TokenReview allow-list, set in deploy values) plus a CiliumNetworkPolicy as defence in depth; the residual exposure is fail-open if FC_INVOKE_ALLOWED_CALLERS is unset, and TokenReview validates the API server default audiences rather than a dedicated fc-invoke audience.
</details>
