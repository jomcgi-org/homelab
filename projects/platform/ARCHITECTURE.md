...
**Edge caching.** Anonymous public pages and data endpoints opt into shared caching with explicit origin headers. The observability snapshot uses `Cache-Control: public, max-age=0, s-maxage=60, stale-while-revalidate=86400, stale-if-error=31536000`; the frontend emits a separate Cloudflare-only policy so `s-maxage` does not disable stale serving. The cache rule and alert remain scoped to `public.jomcgi.dev`, while private and authenticated hostnames are excluded. `projects/platform/cloudflare-cache/` declares Browser Cache TTL as Respect Existing Headers, Smart Tiered Cache on, Always Online on, the required public-only rule shape, and a greater-than-5-percent MISS alert over 15 minutes using Cloudflare Analytics GraphQL. Repository state is implemented, but no scheduler currently invokes the reconciler or monitor. Live reconciliation and repeated-request acceptance remain unverified until those commands run and the public-host checks record `cf-cache-status` and `age` (#3876). `jomcgi.dev/docs/*` is served by the monolith frontend from repo markdown (`projects/monolith/frontend/src/routes/public/docs/`); there is no separate docs site or Cloudflare Pages project (ADR docs/002).
...
| CDN cache rule scoped to the public hostname | platform/003 | Repository contract implemented for `public.jomcgi.dev`; live acceptance unverified, #3876 | deleted |
