...
Two clusters, one repository. **The GKE hub** (`homelab-hub`, a zonal Standard cluster in `europe-west2`) has served every public and private workload since the 2026-08-31 cutover: the monolith with its public and agents tiers, EmberVM, Context Forge, authentik, the ingress, and the inference embeddings pod. It remains the always-on, more reliable hub. The repository also carries the home k3s configuration and its last recorded residual inference/platform placement. The hub is the sole management plane: the plan to bring already-owned nodes back as UK home capacity was dropped on 2026-09-18 because the household is moving ([#4964](https://github.com/jomcgi-org/homelab/issues/4964) closed, not planned). The former teardown and GKE GPU proposals ([#5485](https://github.com/jomcgi-org/homelab/issues/5485), [#5461](https://github.com/jomcgi-org/homelab/issues/5461)) closed as not planned and authorize no hardware disposal.
...
**The hub's CNI is GKE Dataplane V2**, Google's managed Cilium (`anetd`). This repo's Cilium chart is not deployed there. A read-only check on 2026-09-13 found `anetd` ready on two nodes and Cilium endpoint, endpoint-slice, identity, node, and local-redirect-policy CRDs, but no `CiliumNetworkPolicy` or `CiliumClusterwideNetworkPolicy` CRDs and no Hubble deployment in `kube-system`. These are observations of the current hub configuration, not permanent limitations of GKE.

Google supports optional [Cilium cluster-wide network policies](https://docs.cloud.google.com/kubernetes-engine/docs/how-to/configure-cilium-network-policy) and [Hubble-based Dataplane V2 observability](https://docs.cloud.google.com/kubernetes-engine/docs/concepts/about-dpv2-observability). Its Cilium integration does not support Layer 7 policies such as HTTP method/path filtering. An absent Hubble deployment therefore does not prove that GKE cannot provide Hubble, while enabling a supported network-policy feature does not supply the HTTP enforcement required by ADR networking/003. Before escalating a task as unsupported, check the current cluster resources and the relevant Google feature limitations separately. Enabling a feature or changing enforcement placement still needs the task's scope and rollout requirements to be satisfied.

The charts' Cilium policies remain switched off in their `values-gke.yaml` overlays (monolith, monolith-public, monolith-agents, and EmberVM's noded and token broker), because the current hub lacks the policy CRDs those templates require. Standard Kubernetes `NetworkPolicy` is available for L3/L4 pod-network enforcement; application or proxy authorization is a separate enforcement surface. The cutover did not carry over home's WireGuard configuration, and nothing here claims hub wire encryption. The networking/003 programme closed with #3875 (default-deny audit, not planned) and #3824 (L7 Part A); hub default-deny is tracked with plain `NetworkPolicy` under #3897 and #5277. The absence of optional CRDs or observability on the hub is not by itself grounds to call new network-policy work impossible.


**Home's checked-in configuration** runs the chart under `projects/platform/cilium/`, which replaced Linkerd's sidecar mesh (ADR platform/012): eBPF L3/L4 `CiliumNetworkPolicy` enforcement in allow-until-selected mode, pod-to-pod WireGuard on `cilium_wg0` (node-to-node encryption was set and never in force, #5146 closed as obsolete), Hubble flow logs with cert-manager-rotated mTLS, and kube-proxy replacement. The closed #5485 decision no longer supplies a teardown schedule, and with #4964 closed no live cluster runs this configuration; it is recorded for the residual home checkout only: Cilium enforces egress policy after DNAT, so a probe from an unlisted namespace is dropped as a silent timeout rather than refused, and its `tcx` attachment is what makes the CAKE shaper in section 6 inert.
...
**1Password Operator** syncs external credentials via `OnePasswordItem` CRs on both clusters. The Connect API server and the operator are still bootstrapped out of band before the hub root syncs, because the root's first sync renders items, but on the hub the `onepassword` Application (wave 0) now adopts those resources and manages them from Git. That gives the Connect API two replicas, a `PodDisruptionBudget` (`maxUnavailable: 1`, so a node drain keeps one server up), memory requests equal to their limits, and a soft anti-affinity that spreads the replicas; a single-replica Connect was a cluster-wide secret-delivery single point of failure, since evicted or drained no pod that consumes a `OnePasswordItem` could start. The operator stays at one replica on purpose: the chart enables no leader election, so a second reconciler would race on the same items. Nothing external is hand-created; the bootstrap Secrets (`op-credentials`, `onepassword-token`) stay out of Git, and the chart renders neither unless its values supply them. Secrets generated inside the cluster (CloudNativePG credentials, cert-manager leaf certificates) are owned by their operators.
...
**Argo Workflows** runs in `monolith-workflows` on both clusters as the CronWorkflow executor. On the hub it runs the monolith's job schedule (32 CronWorkflows); at home it runs Renovate and apko lock maintenance, with the hub enrolment staged and suspended (#6247).
...
| --- | --- | --- | --- |
| Plain `NetworkPolicy` replaces the inert Cilium templates on the hub | section 3 | #5816, #3897, #5277 | hub-only since #4964 closed 2026-09-18; #5485 closed without teardown |
...
| CDN cache rule scoped to the public hostname | platform/003 | Implemented on the apex; Cloudflare-side only, remaining dashboard settings #3879 | deleted |
