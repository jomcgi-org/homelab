...
| -------------- | ------------------------------------------------------------------------------------------ |
| **backend**    | FastAPI server that replays trip data from NATS JetStream and serves REST + WebSocket APIs (`GET /api/points`, `GET /api/points/{point_id}`, `GET /api/stats`, `GET /health`, `WS /ws/live`); image ingestion is handled by the `publish-trip-images` CLI tool, not the backend |
