...
bounded probe timeout; otherwise, and only when no bank or active splice owns
the entry, it withdraws and tears down that endpoint before following the
...
operator check. The repository carries an unreferenced, default-off dev preset for
...
[agent-substrate/substrate at its cited 2026-06-25 revision](https://github.com/agent-substrate/substrate/blob/8631ae34655f73895c811bffa6cb9a30f1792d0e/docs/threat-model.md),
...
| Default-deny actor networking (17) | **Built** for task and session guests: they have no NIC and use vsock only. For tap-bearing serving and stateful guests, the generated serving table accepts established/related replies and drops NEW traffic entering the shared serving bridge at the `inet` forward hook. Whether two guests on that bridge can exchange bridge-local unicast, broadcast, ARP, or IPv4 traffic is runtime-unverified (#6159), not established by the generated inet rules. Composite members intentionally share a per-group L2 bridge: the generated group table keeps same-bridge member traffic while dropping NEW traffic that leaves the group bridge. That is composite lifecycle connectivity, not independent service authorization. #5813 is closed as NOT_PLANNED, and EmberVM ships no platform VM-to-VM L7 authorization layer. Brokered external egress remains the deliberate exception described in section 9. |
| No guest access to node-local endpoints or local substrate services (16, 19) | **Built** for task and session guests: with no NIC, their platform path is the vsock shim and brokered egress contract. **Source-level gap** for tap-bearing serving and stateful guests: `noded/serving/net.go` generates no input hook and no per-tap source IPv4 or source MAC match. Its forward-hook drop therefore does not prove denial of the bridge gateway, noded pod IP, or other host-local listeners, and does not prove that a packet can be attributed to one VM. Installed rules, actual listener reachability, authentication behavior, spoof outcomes, and deployed hook traversal remain live checks in #6159. |
...
| Mutual authentication and encrypted transport between components (4, 10) | **Built** for CP-to-noded authentication: one bearer Secret rendered into the control plane and every noded pod, attached to every gRPC request, enforced in production and dev (#4693). The inert noded and token-broker Cilium policies were removed in #5816. The hub has no corresponding CRDs; the bearer remains the noded authentication control, not a substitute for network isolation. SPIFFE X.509-SVID mTLS is the additive upgrade the proto reserves, in flight as phase 2 of #5706 (section 9). Encrypted session-routing tokens and the actor / principal / permission split are **Decided direction**; management callers authenticate via Kubernetes TokenReview against an allow-list. |
