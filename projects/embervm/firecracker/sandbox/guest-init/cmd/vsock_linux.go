...
// (projects/embervm/firecracker/semgrep/guest-init/internal/scanserver), kept as a
// private helper here rather than a shared package since it is this small.
func listenVsock(port uint32) (net.Listener, error) {
	fd, err := unix.Socket(unix.AF_VSOCK, unix.SOCK_STREAM, 0)
	if err != nil {
		return nil, fmt.Errorf("sandbox-guest-init: vsock socket: %w", err)
	}
	if err := unix.Bind(fd, &unix.SockaddrVM{CID: unix.VMADDR_CID_ANY, Port: port}); err != nil {
		_ = unix.Close(fd)
		return nil, fmt.Errorf("sandbox-guest-init: vsock bind port=%d: %w", port, err)
	}
	if err := unix.Listen(fd, 16); err != nil {
		_ = unix.Close(fd)
		return nil, fmt.Errorf("sandbox-guest-init: vsock listen port=%d: %w", port, err)
	}
	return &vsockListener{fd: fd, port: port}, nil
}

// vsockListener wraps a raw AF_VSOCK listening fd as a net.Listener so the
...
func (l *vsockListener) Accept() (net.Conn, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("sandbox-guest-init: vsock accept: %w", err)
	}
