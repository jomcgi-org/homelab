defmodule Embervm.K8s do
  ...
  is present (in-cluster) the default pool verifies the peer against that pinned
  Kubernetes CA; when it is absent (local `mix`, ExUnit) the plain default pool is
  used, which is why the loopback HTTP smoke and request tests need no cluster.
  """
  require Logger
  ...
  @doc """
  The Finch child spec for the control plane's Kubernetes HTTP pool. In-cluster
  it pins the default pool to the service-account CA bundle; elsewhere it is a
  plain pool. Public object-store traffic uses `Embervm.StoreFinch` instead.
  """
    ...
    pools =
      case File.exists?(ca_file) do
        true ->
          ...
          # map crashes the pool at boot). This pool is deliberately private-CA
          # only. Public object-store trust belongs to Embervm.StoreFinch.
          %{
            default: [
              conn_opts: [
                transport_opts: [verify: :verify_peer, cacertfile: ca_file]
              ]
            ]
          }
