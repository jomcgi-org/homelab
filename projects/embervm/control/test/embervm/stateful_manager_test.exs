defmodule Embervm.StatefulManagerTest do
  ...
  defp start_stack(opts \\ []) do
    ...
    registry =
      if opts[:grace_registry] do
        start_grace_registry(cap_table, Keyword.fetch!(opts, :clock), store)
      end
  ...
  defp stateful_node(ctx, node_id, opts \\ []) do
    ...
    fact =
      case Keyword.fetch(opts, :instance_id) do
        {:ok, instance_id} -> Map.put(fact, :instance_id, instance_id)
        :error -> fact
      end

    NodeCapacity.put(ctx.cap_table, node_id, fact)
  end

  defp instance_only_channel_fun do
    fn key ->
  ...
  # Registration opens a deliberately blocked fake WatchNode stream, and each
  # test injects NodeStatus explicitly so no wall-clock sleep decides when the
  # capacity fact appears.
  defp start_grace_registry(cap_table, clock, stateful_store) do
    {:ok, registry} =
  ...
  defp register_grace_node(ctx) do
    :ok =
  ...
  test "slow in-flight start shares one StartStateful result with second and third wakes" do
    ...
    first = :gen_server.send_request(ctx.mgr, {:wake, "wl-a", "p1"})
    assert_receive {:start_stateful_waiting, worker}

    second = :gen_server.send_request(ctx.mgr, {:wake, "wl-a", "p2"})
    third = :gen_server.send_request(ctx.mgr, {:wake, "wl-a", "p3"})
    ...
    assert {:reply, ^expected} = :gen_server.receive_response(first, 1_000)
    assert {:reply, ^expected} = :gen_server.receive_response(second, 1_000)
    assert {:reply, ^expected} = :gen_server.receive_response(third, 1_000)
    assert Agent.get(calls, & &1) == 1
  end
  ...
  test "joiner timeout does not abort the shared in-flight StartStateful" do
    ...
    owner = :gen_server.send_request(ctx.mgr, {:wake, "wl-a", "owner"})
    assert_receive {:start_stateful_waiting, worker}

    joiner = :gen_server.send_request(ctx.mgr, {:wake, "wl-a", "timed-out-joiner"})
    ...
    assert :timeout = :gen_server.receive_response(joiner, 0)
    send(worker, :complete_start)

    assert {:reply, {:ok, %{ip: "10.88.0.5", port: 5432, generation: 1}}} =
             :gen_server.receive_response(owner, 1_000)
  ...
  describe "boot-relative endpoint grace" do
    test "a serving stateful instance within the grace window never loses its endpoint" do
      ...
      ctx = start_stack(clock: clock, grace_registry: true)
      ...
      {registry, instance_id} = register_grace_node(ctx)
      ...
      # grace because its owner's latest report is still recent.
      Agent.update(now, fn _ -> 90_001 end)
      :ok = StatefulManager.reconcile(ctx.mgr)
      assert StatefulStore.published_endpoint(ctx.store, "wl-grace") == endpoint
    end

    test "a recently reporting node keeps a post-status RPC wake endpoint" do
      {:ok, now} = Agent.start_link(fn -> 0 end)
      clock = fn -> Agent.get(now, & &1) end
      ctx = start_stack(clock: clock, grace_registry: true)

      {registry, instance_id} = register_grace_node(ctx)
      Agent.update(now, fn _ -> 20_000 end)
      ...
      # The wake completed by RPC after the last NodeStatus, so registry expiry's
      # resident inventory cannot name this VM. While that report is still recent,
      # loss of the capacity projection must not withdraw its endpoint.
      NodeCapacity.drop(ctx.cap_table, {"node-4", "grace-pod"})
      Agent.update(now, fn _ -> 109_999 end)
      :ok = StatefulManager.reconcile(ctx.mgr)
      assert StatefulStore.published_endpoint(ctx.store, "wl-grace") == endpoint
    end

    test "a fresh control plane preserves an unreported endpoint until the boot floor" do
      {:ok, now} = Agent.start_link(fn -> 0 end)
      clock = fn -> Agent.get(now, & &1) end
      ctx = start_stack(clock: clock, grace_registry: true)
      ...
      {_registry, _instance_id} = register_grace_node(ctx)
    ...
    test "a stale report withdraws a post-status RPC wake endpoint" do
      {:ok, now} = Agent.start_link(fn -> 0 end)
      clock = fn -> Agent.get(now, & &1) end
      ctx = start_stack(clock: clock, grace_registry: true)
      ...
      # This status predates the RPC wake and therefore cannot put the VM in the
      # registry expiry path's resident inventory.
      :ok =
        NodeRegistry.inject_status(registry, instance_id, %NodeStatus{
          node_id: "node-4",
          stateful_vms: []
      ...
      Agent.update(now, fn _ -> 110_001 end)

      # Registry expiry sees no resident VM in its last status, so Half A cannot
      # withdraw the endpoint created after that status.
  ...
  test "delete_volume skips an anchor absent from capacity and reports it as unreachable" do
    ...
    # A skipped node beyond the quiet window is dialed best-effort; a failed dial
    # (channel error) abandons it without a DeleteVolume RPC being attempted.
  ...
  test "delete_volume answers best-effort delete from a quiet-window node and removes it from unreachable" do
    {:ok, now} = Agent.start_link(fn -> 1_000 end)
    {:ok, deleted} = Agent.start_link(fn -> [] end)
    {:ok, evicted} = Agent.start_link(fn -> [] end)
    clock = fn -> Agent.get(now, & &1) end
    ...
    assert StatefulStore.get_volume(ctx.store, "wl-a") == nil
    assert [{"node-1", delete_req}] = Agent.get(deleted, & &1)
    assert delete_req.workload == "wl-a"
  ...
  test "delete_volume reaches an unreachable anchor through a different reporting node" do
    {:ok, now} = Agent.start_link(fn -> 1_000 end)
    {:ok, deleted} = Agent.start_link(fn -> [] end)
    {:ok, evicted} = Agent.start_link(fn -> [] end)
    clock = fn -> Agent.get(now, & &1) end
    ...
    ctx =
      start_stack(
        ...
        delete_volume_fun: fn node_id, req ->
          Agent.update(deleted, &[{node_id, req} | &1])
          if node_id == "node-1/pod-x", do: {:error, :boom}, else: {:ok, %{}}
        end,
        evict_artifact_fun: fn node_id, req ->
          Agent.update(evicted, &[{node_id, req} | &1])
          {:ok, %{}}
    ...
    stateful_node(ctx, "node-1",
      instance_id: "node-1/pod-x",
      volumes: [%{workload: "wl-a", generation: 1, size_bytes: 10, allocated_bytes: 1}]
    )

    stateful_node(ctx, "node-2", instance_id: "node-2/pod-x", store_reachable: true)
    StatefulStore.upsert_volume(ctx.store, "wl-a", %{node_id: "node-1", generation: 1})
    :ok = StatefulManager.reconcile(ctx.mgr)
    ...
    assert StatefulStore.get_volume(ctx.store, "wl-a") == nil
    assert [{"node-1/pod-x", _delete_req}] = Agent.get(deleted, & &1)

    assert [{"node-2/pod-x", req}] = Agent.get(evicted, & &1)
    assert req.remote
    assert req.artifact.kind == :ARTIFACT_KIND_VOLUME
    assert req.artifact.workload == "wl-a"
  end

  test "evict_remote_volume dials the anchor when it is reporting" do
    {:ok, evicted} = Agent.start_link(fn -> [] end)

    ctx =
      start_stack(
        channel_fun: instance_only_channel_fun(),
        delete_volume_fun: fn _node_id, _req -> {:ok, %{}} end,
        evict_artifact_fun: fn node_id, req ->
          Agent.update(evicted, &[{node_id, req} | &1])
          {:ok, %{}}
        end
    ...
    stateful_node(ctx, "node-1",
      instance_id: "node-1/pod-x",
      volumes: [%{workload: "wl-a"}],
      store_reachable: true
    )

    stateful_node(ctx, "node-2",
      instance_id: "node-2/pod-x",
      volumes: [%{workload: "wl-a"}],
      store_reachable: true
    )
    ...
    assert [{"node-1/pod-x", req}] = Agent.get(evicted, & &1)
    assert req.remote
    assert req.artifact.kind == :ARTIFACT_KIND_VOLUME
    assert req.artifact.workload == "wl-a"
  end

  test "delete_volume retains unreachable node when best-effort delete RPC errors" do
    {:ok, now} = Agent.start_link(fn -> 1_000 end)
    {:ok, evicted} = Agent.start_link(fn -> [] end)
    clock = fn -> Agent.get(now, & &1) end
    ...
    ctx =
      start_stack(
        clock: clock,
        channel_fun: instance_only_channel_fun(),
        delete_volume_fun: fn
          "anchor-node/pod-x", _req -> {:error, :boom}
          _node_id, _req -> {:ok, %{}}
        end,
        evict_artifact_fun: fn node_id, req ->
          Agent.update(evicted, &[{node_id, req} | &1])
    ...
    stateful_node(ctx, "anchor-node",
      instance_id: "anchor-node/pod-x",
      volumes: [%{workload: "wl-a", generation: 1, size_bytes: 10, allocated_bytes: 1}]
    )

    StatefulStore.upsert_volume(ctx.store, "wl-a", %{node_id: "anchor-node", generation: 1})
    :ok = StatefulManager.reconcile(ctx.mgr)

    NodeCapacity.drop(ctx.cap_table, "anchor-node")
    ...
    assert {:ok, %{deleted: true, unreachable: ["anchor-node"]}} =
             StatefulManager.delete_volume(ctx.mgr, "wl-a")

    assert StatefulStore.get_volume(ctx.store, "wl-a") == nil
    assert [{"node-2/pod-x", _req}] = Agent.get(evicted, & &1)
  end

  test "delete_volume invalidates channel on best-effort delete exit" do
    {:ok, now} = Agent.start_link(fn -> 1_000 end)
    {:ok, invalidated} = Agent.start_link(fn -> [] end)
    clock = fn -> Agent.get(now, & &1) end
    ...
    ctx =
      start_stack(
        clock: clock,
        channel_fun: instance_only_channel_fun(),
        invalidate_fun: fn node_id, channel ->
          Agent.update(invalidated, &[{node_id, channel} | &1])
          :ok
        end,
        delete_volume_fun: fn
          "anchor/pod-x", _req -> exit(:noproc)
          _node_id, _req -> {:ok, %{}}
        end,
        evict_artifact_fun: fn _node_id, _req -> {:ok, %{}} end
      )

    stateful_workload(ctx, "wl-a")

    stateful_node(ctx, "anchor",
      instance_id: "anchor/pod-x",
      volumes: [%{workload: "wl-a", generation: 1, size_bytes: 10, allocated_bytes: 1}]
    )

    StatefulStore.upsert_volume(ctx.store, "wl-a", %{node_id: "anchor", generation: 1})
    :ok = StatefulManager.reconcile(ctx.mgr)

    NodeCapacity.drop(ctx.cap_table, "anchor")
    ...
    assert {:ok, %{deleted: true, unreachable: ["anchor"]}} =
             StatefulManager.delete_volume(ctx.mgr, "wl-a")

    assert {"anchor/pod-x", "anchor/pod-x"} in Agent.get(invalidated, & &1)
    assert StatefulStore.get_volume(ctx.store, "wl-a") == nil
  end

  test "best-effort delete reaches an anchor that stopped reporting through its last-known dial id" do
    {:ok, now} = Agent.start_link(fn -> 1_000 end)
    {:ok, deleted} = Agent.start_link(fn -> [] end)
    {:ok, evicted} = Agent.start_link(fn -> [] end)
    clock = fn -> Agent.get(now, & &1) end
    ...
    assert [{"node-1/pod-x", delete_req}] = Agent.get(deleted, & &1)
    assert delete_req.workload == "wl-a"
    assert StatefulStore.get_volume(ctx.store, "wl-a") == nil
    assert [{"node-1/pod-x", evict_req}] = Agent.get(evicted, & &1)
    assert evict_req.artifact.workload == "wl-a"
  end

  test "a never-reporting anchor has no dial id and is abandoned" do
    {:ok, now} = Agent.start_link(fn -> 1_000 end)
    {:ok, deleted} = Agent.start_link(fn -> [] end)
    {:ok, evicted} = Agent.start_link(fn -> [] end)
    clock = fn -> Agent.get(now, & &1) end
    ...
    assert {:ok, %{deleted: true, unreachable: ["node-never"]}} =
             StatefulManager.delete_volume(ctx.mgr, "wl-a")

    assert [{"node-2/pod-x", delete_req}] = Agent.get(deleted, & &1)
    assert delete_req.workload == "wl-a"
    assert StatefulStore.get_volume(ctx.store, "wl-a") == nil
    assert [{"node-2/pod-x", evict_req}] = Agent.get(evicted, & &1)
    assert evict_req.artifact.workload == "wl-a"
  end

  test "store eviction skips a reporting node without a reachable store" do
    {:ok, now} = Agent.start_link(fn -> 1_000 end)
    {:ok, evicted} = Agent.start_link(fn -> [] end)
    clock = fn -> Agent.get(now, & &1) end
    ...
    stateful_node(ctx, "node-2", instance_id: "node-2/pod-x")
    stateful_node(ctx, "node-3", instance_id: "node-3/pod-x", store_reachable: true)
    StatefulStore.upsert_volume(ctx.store, "wl-a", %{node_id: "node-1", generation: 1})
    ...
    assert StatefulStore.get_volume(ctx.store, "wl-a") == nil
    assert [{"node-3/pod-x", req}] = Agent.get(evicted, & &1)
    assert req.artifact.workload == "wl-a"
  end

  test "store eviction warns and dials the anchor when no node reports a reachable store" do
    {:ok, now} = Agent.start_link(fn -> 1_000 end)
    {:ok, evicted} = Agent.start_link(fn -> [] end)
    clock = fn -> Agent.get(now, & &1) end
    ...
    ctx =
      start_stack(
        clock: clock,
        channel_fun: instance_only_channel_fun(),
        delete_volume_fun: fn _node_id, _req -> {:error, :boom} end,
        evict_artifact_fun: fn node_id, req ->
          Agent.update(evicted, &[{node_id, req} | &1])
          {:ok, %{}}
        end
    ...
    assert log =~
             "no store-reachable node for store eviction, dialing the anchor best-effort"
  ...
  test "confirmed gone anchor evicts a banked instance and restores its exported volume" do
    {:ok, now} = Agent.start_link(fn -> 0 end)
    clock = fn -> Agent.get(now, & &1) end
    {restore_fun, restore_calls} = recording_restore_fun()
    ...
    stateful_workload(ctx, "wl-a")
    stateful_node(ctx, "node-live", store_reachable: true)
    seed_banked_with_pair(ctx, "stf-dead-anchor", "node-dead", 3, 3)
    ...
    assert Enum.any?(ops, fn op ->
             op.kind == :stateful_evicted and op.stateful_instance_id == "stf-dead-anchor" and
               op.payload["reason"] == "anchor_gone"
           end)
  end
  ...
  test "unconfirmed gone anchor keeps its banked instance and logs the refusal" do
    ...
    log =
      capture_log(
        [format: "$message $metadata\n", metadata: [:workload, :anchor, :missing_since_ms]],
        fn ->
          assert {:error, {:wake_failed, :volume_node_gone}} =
                   StatefulManager.wake(ctx.mgr, "wl-a", "p")
        end
    ...
    assert log =~ "missing_since_ms=0"
    assert Agent.get(restore_calls, & &1) == []

    {:ok, banked} = StatefulStore.get(ctx.store, "stf-dead-anchor")
    assert banked.state == :banked
  end
  ...
  test "manual wake joins an in-flight auto-wake and receives its shared result" do
    ...
    manual = :gen_server.send_request(ctx.mgr, {:wake, "wl-a", "manual"})
    ...
    assert {:reply, {:ok, %{ip: "10.88.0.5", port: 5432, generation: 1}}} =
             :gen_server.receive_response(manual, 1_000)
