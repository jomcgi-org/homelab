...
# leave both signals without exporters, so a run with no collector boots
# cleanly and exports nothing. The endpoint-less case is the safe default for
# local runs.
otlp_endpoint = System.get_env("OTEL_EXPORTER_OTLP_ENDPOINT", "")

if otlp_endpoint != "" do
  unless Code.ensure_loaded?(:otel_exporter_metrics_otlp) do
    raise "configured OTLP metric exporter :otel_exporter_metrics_otlp is not available"
  end
