...
type servingDriver interface {
	...
	// serving base (ADR embervm/038: no handler archive exists). The startup rescan
	// returns the marker as a handler-less candidate; replay-driven reconciliation
	// validates its base, serving class, and current provisioned ref before adding
	// inventory (#5221). Idempotent.
