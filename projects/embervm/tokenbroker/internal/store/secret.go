...
func (s *SecretStore) LoadGrant(name string) (Grant, error) {
	...
	if apierrors.IsNotFound(err) {
		return Grant{}, fmt.Errorf("chart-managed OAuth Secret %q not found", SecretName(name))
	}
	if err != nil {
		return Grant{}, err
	}
	...
	if v, _ := decode("expires_at"); v != "" {
		b.TokenBundle.ExpiresAt, err = time.Parse(time.RFC3339Nano, v)
		if err != nil {
...
func data(b TokenBundle, lastRefresh time.Time) map[string][]byte {
	return map[string][]byte{"id_token": []byte(b.IDToken), "access_token": []byte(b.AccessToken), "refresh_token": []byte(b.RefreshToken), "account_id": []byte(b.AccountID), "last_refresh": []byte(lastRefresh.UTC().Format(time.RFC3339Nano)), "expires_at": []byte(b.ExpiresAt.UTC().Format(time.RFC3339Nano))}
}
...
func (s *SecretStore) SaveGrant(g Grant) error {
	...
	if apierrors.IsNotFound(err) {
		return fmt.Errorf("chart-managed OAuth Secret %q not found", SecretName(g.Name))
	}
	if err != nil {
		return err
	}
...
var ErrGrantNotNewer = errors.New("grant_not_newer")
...
func (s *SecretStore) SaveGrantIfNewer(g Grant) error {
	...
	currentLast, err := time.Parse(time.RFC3339Nano, string(current.Data["last_refresh"]))
	if err != nil && len(current.Data["last_refresh"]) != 0 {
		return fmt.Errorf("parse last_refresh: %w", err)
	}
	if !currentLast.IsZero() && !g.LastRefresh.After(currentLast) {
		return ErrGrantNotNewer
	}
	current.Data = data(g.TokenBundle, g.LastRefresh)
	_, err = secrets.Update(s.ctx(), current, metav1.UpdateOptions{})
	return err
