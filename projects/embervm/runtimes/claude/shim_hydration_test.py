...
def test_failed_hydration_retries_on_next_turn_and_succeeds(
    ...
    assert all(
        command[-2] == "https://github.com/owner/repo.git" for command in clone_calls
    )
...
def test_git_clone_regression_guards(manager, monkeypatch):
    ...
    assert "--depth=1" not in command, "github clone must not be shallow"
    assert "--filter=blob:none" in command, "github clone missing --filter=blob:none"
    assert "--single-branch" in command, "github clone missing --single-branch"
    assert any(arg.startswith("http.proxy=") for arg in command), (
        "github clone missing http.proxy config"
    )
    assert not any(arg.startswith("git://") for arg in command), (
        "github clone must not use the git:// transport"
    )
    assert any("extraHeader" in arg for arg in command), (
        "github clone lost its credential opt-in"
    )
    assert command[-2] == "https://github.com/owner/repo.git"
...
def test_clone_sends_an_authorization_header_to_opt_into_injection(
    ...
    This guard pins the direct GitHub attempt, where the opt-in belongs.
    """
    ...
    assert len(commands) == 1, "hydration must make one direct GitHub clone"
    configs = [
        arg for arg in commands[0] if arg.startswith("http.") and "extraHeader" in arg
    ]
    assert len(configs) == 1, "github clone must set exactly one extraHeader"
    config = configs[0]
