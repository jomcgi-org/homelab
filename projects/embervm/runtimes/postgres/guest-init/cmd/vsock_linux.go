...
func listenVsock(port uint32) (net.Listener, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("ember-postgres-init: vsock socket: %w", err)
	}
	if err := unix.Bind(fd, &unix.SockaddrVM{CID: unix.VMADDR_CID_ANY, Port: port}); err != nil {
		_ = unix.Close(fd)
		return nil, fmt.Errorf("ember-postgres-init: vsock bind port=%d: %w", port, err)
	}
	if err := unix.Listen(fd, 16); err != nil {
		_ = unix.Close(fd)
		return nil, fmt.Errorf("ember-postgres-init: vsock listen port=%d: %w", port, err)
	}
...
func (l *vsockListener) Accept() (net.Conn, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("ember-postgres-init: vsock accept: %w", err)
	}
