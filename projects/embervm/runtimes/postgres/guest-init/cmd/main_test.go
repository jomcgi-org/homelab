...
// the python runtime guest-init: base64url decoding, an invalid key skipped, a
// malformed base64 value skipped, an absent prefix ignored, and duplicate keys
// (last wins). This is the seam that delivers POSTGRES_PASSWORD to first boot.
func TestMmdsEnvFromCmdline(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{
			"postgres password",
			"console=ttyS0 ember.env.POSTGRES_PASSWORD=" + pw,
			map[string]string{"POSTGRES_PASSWORD": "hunter2"},
		},
...
// POSTGRES_PASSWORD token sets it in the process env; an absent token or missing
...
func TestSetMmdsEnv(t *testing.T) {
	...
	t.Run("password token present sets env", func(t *testing.T) {
		...
		pw := base64.RawURLEncoding.EncodeToString([]byte("hunter2"))
		if err := os.WriteFile(path, []byte("init=/init ember.env.POSTGRES_PASSWORD="+pw+"\n"), 0o600); err != nil {
			t.Fatal(err)
		}
	...
	t.Run("absent leaves env unset (base build / relight)", func(t *testing.T) {
		...
		if _, set := os.LookupEnv("POSTGRES_PASSWORD"); set {
			t.Fatalf("POSTGRES_PASSWORD set, want unset")
		}
	})
}

// TestStatefulVolumeFromCmdline covers the boot-class discriminator: the
// presence of ember.volume_dev distinguishes a stateful cold boot (mount +
// launch Postgres) from a base build (vsock ready only, no volume/Postgres).
