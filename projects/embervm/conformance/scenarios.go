...
const elixirUnicodeSource = `IO.puts("non-ascii round trip: #{String.upcase("café αβγ")}")`
...
func runS5(ctx context.Context, cfg config, client *controlPlaneClient, suiteStarted time.Time) scenarioVerdict {
	...
	const expected = "non-ascii round trip: CAFÉ ΑΒΓ\n"
	if guest.ExitCode != 0 || guest.Stdout != expected || guest.Stderr != "" || guest.Error != "" {
		return scenarioVerdict{Verdict: verdictFail, Detail: fmt.Sprintf(
			"guest exit_code=%d stdout=%q stderr=%q error=%q",
			guest.ExitCode,
	...
	return scenarioVerdict{Verdict: verdictPass, Detail: fmt.Sprintf("guest exited 0 with exact stdout %q and empty stderr; VM reap observed in %s", expected, reapDelay.Round(time.Millisecond))}
}
