...
| **backend**  | REST + WebSocket API over SQLite with stationary-vessel deduplication (speed < 0.5 kn, distance < 100 m, and < 300 s elapsed); uses a durable NATS consumer (`ships-api`) so restarts resume from the last acknowledged message rather than replaying the full stream, then subscribes for live updates |
