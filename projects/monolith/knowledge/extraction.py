...
def _lens(raw: RawInput) -> str:
    ...
    if raw.source == "codex-session":
        ...
        return (
            ...
            f"about:\n{failure_modes}\n\n"
            "Render each failure mode as a causal or constraint fact in one of the "
            "base shapes: for example, `when the worker hit exit 42 quota after 12 "
            "tool calls, the dispatch ended with no diff`, never as a bare event "
            "label. Do not use the failure-mode titles as atom titles; a title names "
            "the specific fact. "
