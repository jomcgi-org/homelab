...
_DEFAULT_SYSTEM_PROMPT = (
    ...
    "Each public note is prefixed with `state=` indicating whether it is verified "
    "or unverified by the knowledge system, and `disputed=true` if it is "
    "contested. Unverified facts and disputed notes must be qualified in your "
    "answer. "
...
def _format_retrieved_context(retrieved: list[RetrievedNote]) -> str:
    ...
    """
    blocks = "\n\n".join(
        f"[note state={note.verification_state} "
        f"disputed={str(note.disputed).lower()}: {note.title}]\n{note.chunk_text}"
