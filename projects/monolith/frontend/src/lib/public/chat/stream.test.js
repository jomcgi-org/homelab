...
describe("applyFrame (turn-state reducer)", () => {
  ...
  it("collects node_touched into the highlight set, deduped and ordered", () => {
    let s = initialTurnState();
    s = applyFrame(s, { type: "node_touched", data: { id: 7, title: "Loom" } });
    s = applyFrame(s, { type: "node_touched", data: { id: 3, title: "STPA" } });
...
describe("streamChatMessage", () => {
  it("POSTs the message to the SSR proxy and delivers parsed frames in order", async () => {
    const fetchImpl = vi.fn().mockResolvedValue({
      ...
      status: 200,
      body: streamFrom([
        f("node_touched", { id: 9, title: "Sextant" }),
        f("token", { text: "hi" }),
  ...
  it("reduces a full stream into transcript + highlight set via applyFrame", async () => {
    const fetchImpl = vi.fn().mockResolvedValue({
      ok: true,
      status: 200,
      body: streamFrom([
        ...
        f("token", { text: "two " }) + f("token", { text: "notes" }),
    ...
    expect(s.assistant).toBe("two notes");
