// Client-side public-chat turn streaming (ADR 005, Phase 4b).
//
// The browser never talks to the internal chat API: it POSTs the single user
// message to the same-origin SSR proxy (/chat/message, rerouted to
// /public/chat/message), which forwards it (session id comes from the httpOnly
// cookie, never the body) and passes the upstream SSE stream straight back. This
// module is the single, testable seam that POSTs the message, parses the SSE
// frames, and reduces them into the chat view state, so the Svelte component
...
// Same-origin SSR proxy path (the reroute hook maps it to /public/chat/message).
export const MESSAGE_PROXY_PATH = "/chat/message";

// Mirrors the backend CHAT_PUBLIC_CHAR_CAP default (limits.py). The server is
// authoritative; this is only a courtesy ceiling on the textarea so a user does
// not compose a message the backend will reject with a 400 char_cap.
...
const FALLBACK_BUSY =
  "The graph is busy right now. Give it a moment and try again.";
 ...
 *
 * Pure: no DOM, no I/O. The `node_touched` reducer is what drives the graph
 * overlay highlight, so it dedupes by id and preserves arrival order.
 *
