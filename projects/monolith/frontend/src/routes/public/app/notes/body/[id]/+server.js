...
// Same-origin proxy for the public note body. The notes graph view fetches this
// site route (/app/notes/body/<id>) when a graph node is selected; the fetch to
// the backend's /api/knowledge/public/notes/<id> happens server-side here, so
// the browser never calls the backend directly. This keeps the public API
// entirely off the internet: the gateway only ever routes to the frontend, and
