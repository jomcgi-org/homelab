<script>
  ...
  const MODEL_NAME = "MUSE SPARK 1.3"; // muse-spark-1.3-contributor
  const BOT_LABEL = "MUSE SPARK / META";
  ...
  onMount(() => {
    ...
    // link lands directly on that graph-focused view. NotesApp only mounts in
    // the browser, so $page is safe to read here. The chat transcript is never
