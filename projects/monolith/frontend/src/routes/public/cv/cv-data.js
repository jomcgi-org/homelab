...
export const projects = [
  ...
  `**Semgrep scans on agent diffs** ([projects/embervm/firecracker/semgrep](https://github.com/jomcgi/homelab/tree/main/projects/embervm/firecracker/semgrep)): a snapshot-warm guest with a resident Semgrep engine scans the code the agents write, synchronously: **~${semgrepRestoreMs}ms restore, ~${semgrepScanSec}s for a full scan** with taint analysis against a ~${semgrepColdStartSec}s cold start, so the scanner sits inside the agent's write path instead of a CI queue.`,
