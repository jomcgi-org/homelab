...
describe("factory board page", () => {
  test("polls the canonical factory board endpoint", async () => {
    vi.useFakeTimers();
    ...
    renderPage({ board: board(), task: null, error: false });
    await tick();
  ...
  test("opening a task shows its nodes, sessions, and the conductor link", async () => {
    ...
    expect(session.getAttribute("href")).toBe(
      "/factory/execution?session=3540",
    );
    ...
    expect(href.pathname).toBe("/factory/execution");
  ...
  test("opening a finished card without a plan fetches it with ?task=", async () => {
    ...
    await tick();
    await vi.waitFor(() =>
      expect(fetchMock).toHaveBeenCalledWith("/factory?task=t-0"),
    );
  });
