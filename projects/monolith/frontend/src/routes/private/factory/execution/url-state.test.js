...
describe("agent URL state", () => {
  test("parses URL and returns null for absent params", () => {
    expect(
      parseUrlState(
        new URL(
          "/private/factory/execution?run=wf-123",
          "https://example.test",
        ),
      ),
    ).toEqual({
  ...
  test("keeps the path the browser is on rather than the route id", () => {
    // private.jomcgi.dev serves this page at /agents; /private/factory/execution is only
    // the internal route src/hooks.js reroutes onto. Navigating to the route id
    // rewrote the address bar to a URL nobody types.
    expect(withSearch("/factory/execution", "run=wf-123")).toBe(
      "/factory/execution?run=wf-123",
    );
  });

  test("drops the question mark when nothing is selected", () => {
    expect(withSearch("/factory/execution", "")).toBe("/factory/execution");
  });

  test("composes with the transitions it exists to serve", () => {
    const search = selectSession(selectRun("", "wf-1"), "wf-1-call2");
    expect(withSearch("/factory/execution", search)).toBe(
      "/factory/execution?run=wf-1&session=wf-1-call2",
    );
  });
  ...
  test("encodes ids and round-trips them", () => {
    ...
    expect(
      parseUrlState(
        new URL(`/private/factory/execution?${search}`, "https://example.test"),
      ),
    ).toEqual({
      runId: "wf-parent",
      sessionId,
