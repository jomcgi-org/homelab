<script>
  ...
  import { laneSummary } from "../factory-view.js";
  import {
    arrivalSelection,
  ...
  // True only after /factory/execution/runs has succeeded at least once. A deep-link
  ...
  async function loadDetail(
    ...
    try {
      ...
      const response = await fetch(
        `/factory/execution/session/${encodeURIComponent(id)}${suffix}`,
      );
  ...
  async function loadSessions() {
    try {
      const response = await fetch("/factory/execution/data");
    ...
    for (let attempt = 0; attempt < RUNS_LOAD_MAX_ATTEMPTS; attempt++) {
      try {
        const responses = await Promise.all([
          fetch("/factory/execution/runs?active=true"),
          fetch("/factory/execution/runs?active=false"),
        ]);
  ...
  async function loadDrainLane() {
    try {
      const response = await fetch("/factory/execution/drain-lane");
      if (!response.ok) return;
  ...
  async function loadRunDetail(
    ...
    try {
      const response = await fetch(
        `/factory/execution/runs/${encodeURIComponent(id)}`,
      );
  ...
  // `/private/factory/execution` is the internal route id, not the address the browser is
  // on: the reroute hook (src/hooks.js) maps private.jomcgi.dev/factory/execution onto it,
  ...
  async function loadVms() {
    try {
      const response = await fetch("/factory/execution/vms");
  ...
  async function loadRepos() {
    repoLoading = true;
    try {
      const response = await fetch("/factory/execution/repos");
  ...
  async function loadBranches(repoId) {
    ...
    try {
      const response = await fetch(
        `/factory/execution/repos/${encodeURIComponent(owner)}/${encodeURIComponent(repo)}/branches`,
      );
  ...
  async function runSearch(value = searchQuery) {
    ...
    try {
      const response = await fetch(
        `/factory/execution/search?q=${encodeURIComponent(query)}&limit=30`,
        {
  ...
  async function sendSessionPrompt({ session_id, prompt: text, model }) {
    ...
    const response = await fetch(
      `/factory/execution/session/${encodeURIComponent(session_id)}/messages`,
      {
        method: "POST",
  ...
  async function decideVoiceRun({ workflowId, nodeKey, decision, note = "" }) {
    ...
    try {
      const response = await fetch(
        `/factory/execution/runs/${encodeURIComponent(workflowId)}/nodes/${encodeURIComponent(nodeKey)}/decision`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
  ...
  async function destroySession() {
    ...
    try {
      const response = await fetch(
        `/factory/execution/session/${encodeURIComponent(selectedId)}`,
        { method: "DELETE" },
      );
  ...
  async function cancelRun(id) {
    ...
    try {
      const response = await fetch(
        `/factory/execution/runs/${encodeURIComponent(id)}/cancel`,
        { method: "POST" },
      );
  ...
  $effect(() => {
    ...
    // /factory/execution form the other fetches here use. The /private prefix is added
    ...
    // /factory/execution/... , matches the catch-all, and never gets the long timeout
    // the stream needs. Envoy then resets it at its 15s default and
    // EventSource reconnects forever, roughly four times a minute.
    const source = new EventSource("/private/factory/execution/vms/stream");
    let failures = 0;
  ...
  async function registerVoiceCompanion(signal = AbortSignal.timeout(10000)) {
    ...
    const response = await fetch("/factory/execution/companion", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  async function loadVoiceSession(
    ...
    const response = await fetch(
      `/factory/execution/session/${encodeURIComponent(sessionId)}${suffix}`,
      { signal },
    );
    if (!response.ok) return;
    const body = await response.json();
  ...
  async function pollVoiceLedger(signal = AbortSignal.timeout(10000)) {
    ...
    const response = await fetch(
      `/factory/execution/companion/${encodeURIComponent(voiceCompanionId)}/ledger?since=${voiceSince}`,
      { signal },
    );
...
>
  <header class="topbar">
    <a class="wordmark" href="/factory">Factory</a>
    {#if voiceMode}
