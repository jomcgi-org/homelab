...
describe("Codex login proxies", () => {
  ...
  test("GET forwards the optional grant with a 10 second timeout", async () => {
    ...
    const response = await GET({
      url: new URL(
        "http://frontend/private/factory/execution/codex-login/status?grant=codex-dev",
      ),
    });
  ...
  test("POST forwards the optional grant with a timeout", async () => {
    ...
    const response = await POST({
      url: new URL(
        "http://frontend/private/factory/execution/codex-login/start?grant=codex-dev",
      ),
    });
  ...
  test.each([
    ...
    const response = await handler({
      url: new URL(
        `http://frontend/private/factory/execution/codex-login/${name}`,
      ),
    });
