<script>
  ...
  async function load() {
    try {
      const response = await fetch("/factory/execution/drain/console");
  ...
  async function loadDetail(name) {
    ...
    try {
      const response = await fetch(
        `/factory/execution/drain/jobs/${encodeURIComponent(name)}`,
      );
      if (!response.ok) throw new Error(`backend ${response.status}`);
  ...
  async function kick() {
    ...
    actionError = null;
    try {
      const response = await fetch("/factory/execution/drain/kick", {
        method: "POST",
      });
      if (!response.ok) throw new Error(`backend ${response.status}`);
      await load();
  ...
  async function requeue(name) {
    ...
    try {
      const response = await fetch(
        `/factory/execution/drain/jobs/${encodeURIComponent(name)}/requeue`,
        { method: "POST" },
      );
...
<div class="console drain-page">
  <div class="wrap">
    <header class="top">
      <a class="back mono" href="/factory/execution"
        >&larr; {D.labels.backToAgents}</a
      >
                    ...
                    <div class="attempt">
                      <div class="attempt-head mono">
                        ...
                        <a
                          ...
                          href={`/factory/execution?session=${attempt.session_id}`}
                        >
