...
def reading(*, force: bool = False) -> dict | None:
    ...
    if not force and _cache is not None and now - _cache[0] < READING_TTL_SECONDS:
        return _cache[1]
    try:
        from factory.execution.provider_quota import fetch_provider_quota_sync
...
def observe(policy: dict, *, session: Session | None = None) -> dict:
    ...
    """
    from factory.orchestration.model_pool import quota_summary, select_reviewer
...
def reviewer_for(
    ...
    """
    from factory.orchestration.model_pool import select_reviewer
