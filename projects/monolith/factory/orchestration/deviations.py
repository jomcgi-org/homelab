...
# factory.orchestration.graph owns the same tuple, but importing it here would drag core.db and
# the engine into every consumer of this pure module.
_SETTLED = ("succeeded", "failed", "escalated", "cancelled")
# factory.orchestration.factory_conductor owns the same pattern, for the same reason the
...
def factory_deviation(
    ...
    *,
    ...
    if pending_review is not None and review_rounds_used >= max_review_rounds:
        ...
        if pending_reason == "queue_ejection":
            text = (
                f"{pending_review} approved a head that was ejected from the merge queue "
                f"after {review_rounds_used} engine-owned correction rounds. "
                "Assess queue and CI evidence before further recovery or escalation."
            )
