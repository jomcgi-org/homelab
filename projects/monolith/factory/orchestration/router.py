...
from factory.orchestration import config, runtime
from factory.orchestration.budget import effective_budget
from factory.orchestration.compare_router import router as compare_router
from factory.orchestration.compare_router import compare_stats
from factory.execution.rationale import parse_rationale
from factory.orchestration.steps import merge_workflow_attributes
from factory.orchestration.walkthrough_composer import compose_walkthrough
...
async def walkthrough(session_id: int, turn_seq: int) -> dict:
    ...
    from factory.orchestration.compare_router import _decode, _turn_data
...
def _session_rows(workflow_id: str):
    ...
    from factory.orchestration.rows import swarm_session_views
...
def _decision_rows(workflow_id: str):
    from core.db import get_engine
    from sqlmodel import Session
    from factory.orchestration.store import list_open_decisions
...
def _decision_rows_for(workflow_ids: list[str]):
    from core.db import get_engine
    from sqlmodel import Session
    from factory.orchestration.store import list_open_decisions_for
...
def _expire_open_decisions_sync(workflow_id: str) -> None:
    from core.db import get_engine
    from sqlmodel import Session
    from factory.orchestration import store
...
def _compose_run_view(dbos, workflow_id: str) -> dict:
    from factory.orchestration.view import compose_run
...
def start_run(request: RunRequest) -> dict:
    ...
    from factory.orchestration.workflows import implement_then_review
...
def _create_task_sync(
    ...
    from core.db import get_engine
    from sqlmodel import Session
    from factory.orchestration import models
...
async def _classify_and_resolve_body(context: _StartContext, progress: dict) -> None:
    ...
    if context.has_classification:
        ...
        from factory.orchestration import classifier
    ...
    from factory.execution.router import StartRequest, start_session_for_task
...
def list_runs(request: Request, active: bool = True, limit: int = 50) -> dict:
    from factory.orchestration.view import compose_master
    from core.db import get_engine
    from sqlmodel import Session
    from factory.orchestration.rows import swarm_session_views
