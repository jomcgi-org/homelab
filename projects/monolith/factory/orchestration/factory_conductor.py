...
from factory.orchestration.models import SwarmConductorCall, SwarmPlanVersion, SwarmTask
...
def planner_prompt(
    ...
    *,
    ...
    return (
        ...
        "When the requested change is already delivered by another merged PR, "
        "propose settle_external with that PR number and its exact head instead "
        "of a duplicate PR: validation requires the PR merged to the task base, "
        "off the task branch, closing the issue, with required checks and an "
        "approving review at the exact head. It settles as externally "
        "fulfilled, never as a factory delivery. "
...
def _planned_fan_ins(task_id: str, policy: dict, nodes: list[dict]) -> int:
    ...
    """
    from factory.orchestration.factory_controls import parallel_limit
...
def _resync_allowance(task_id: str, policy: dict, revision: int) -> None:
    ...
    admit work.
    """
    from factory.orchestration.factory_controls import task_allowance
...
def _record_allowance(task_id: str, policy: dict, cause: str) -> None:
    ...
    from factory.orchestration.factory_controls import record_allowance
...
def _abandon_recovering_factory_session(
    ...
    from factory.execution.reconciliation import _factory_owner, _locked_session
    from factory.orchestration.factory_controls import _audit as _controls_audit
    from factory.orchestration.factory_controls import (
        _locked_session as _factory_locked_session,
    )
...
def _submit_or_reconcile(task: dict, run: dict, dbos) -> None:
    ...
    pin = run["pin"]
    key = pin["workflow_id"]
    from factory.orchestration.factory_attempt_stop import process_attempt_stop
...
def _free_background_slots() -> int:
    from factory.execution.admission import free_background_slots
...
def _slot_budget() -> int:
    ...
    """
    from factory.orchestration.factory_controls import factory_background_reserve
...
def _reviewer_override(
    ...
    """
    from factory.orchestration.factory_controls import window_high
    from factory.orchestration.factory_quota_guard import reviewer_for
    from factory.orchestration.factory_refine import task_class_for
...
def observe_reviewer_routing(policy: dict) -> None:
    ...
    """
    from factory.orchestration.factory_quota_guard import observe
