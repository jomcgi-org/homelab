...
def _quota_span_attributes() -> dict:
    try:  # nosemgrep: no-broad-except-swallow - optional telemetry only
        from factory.execution.provider_quota import (
...
def hold_drainer_job(
    ...
    *,
    ...
    from agent.api import hold_job_for_unknown_outcome
...
def kg_effective_cap(base_cap: int) -> int:
    from core.db import get_engine
    from knowledge.api import kg_effective_cap as _kg_effective_cap
    from sqlmodel import Session
...
def defer_drainer_job(
    ...
    from agent.api import defer_job
...
def update_drainer_job_payload(
    ...
    from agent.api import update_job_payload
...
def finish_drainer_job(
    ...
    *,
    ...
    from agent.api import complete_job, deregister_job
...
def _await_turn(session_id: int, after_seq: int, timeout_seconds: int) -> dict | None:
    from factory.orchestration.workflows import _await_turn as await_turn
...
def chain_next_cycle() -> None:
    ...
    from factory.orchestration.queues import drainer_queue, enqueue_drainer_workers
...
MalformedPayload.__module__ = "swarm.drainer"
InvocationOutcomeUnknown.__module__ = "swarm.drainer"
