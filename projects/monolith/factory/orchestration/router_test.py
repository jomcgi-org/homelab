...
import factory.orchestration.router as swarm_router
from factory.orchestration import store as swarm_store
...
def test_planned_run_resolves_through_status(monkeypatch, start_engine):
    ...
    started = []
    monkeypatch.setattr(
        "factory.orchestration.classifier.classify_task_with_outcome", classify
    )
    monkeypatch.setattr(
        swarm_router,
        "start_run",
...
def test_one_shot_resolves_session_and_login_payload(monkeypatch, start_engine):
    ...
    monkeypatch.setattr(
        "factory.execution.router.start_session_for_task", start_session
    )
    task_id, context = _submit_and_capture(
...
def test_promote_session_carries_model(monkeypatch):
    ...
    monkeypatch.setattr("factory.orchestration.models.mint_task_id", lambda: "task-1")
    monkeypatch.setattr(
        "factory.orchestration.models.create_task", lambda *args, **kwargs: None
    )
    monkeypatch.setattr(
        "factory.orchestration.models.append_plan_version", lambda *args, **kwargs: None
    )
...
def test_compose_run_view_maps_dbos_missing_workflow_errors(monkeypatch, message):
    ...
    monkeypatch.setattr(
        "factory.orchestration.view.compose_run",
...
def test_list_runs_clamps_limit_query_parameter(monkeypatch):
    ...
    monkeypatch.setattr("sqlmodel.Session", FakeSession)
    monkeypatch.setattr(
        "factory.orchestration.rows.swarm_session_views", lambda session: {}
    )
    monkeypatch.setattr(
        "factory.orchestration.store.list_open_decisions_for",
        lambda session, workflow_ids: {},
    )
    ...
    monkeypatch.setattr(swarm_router, "_server_app_version", lambda: "version")
    monkeypatch.setattr("factory.orchestration.view.compose_master", compose)
