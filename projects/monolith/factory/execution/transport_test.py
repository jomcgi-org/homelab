...
from factory.execution import store, transport
...
def test_deliver_creates_spark_session_on_claude_workload(monkeypatch):
    ...
    _client(monkeypatch, handler)
    client = transport.EmberVmShimTransport()
    turn, used = asyncio.run(client.deliver(None, None, "hello", model="spark"))
...
def test_deliver_with_existing_ember_issues_no_create(monkeypatch):
    ...
    ember = transport.EmberSession("s1", "t1", None)
    turn, used = asyncio.run(
        transport.EmberVmShimTransport().deliver(ember, "cli-1", "hello", model="spark")
    )
