...
from factory.execution import execution_api as api
from factory.execution import store
from factory.execution import mcp
from factory.execution import transport
import factory.execution.router as agent_router
from factory.execution.codex_login import codex_login_gate
...
def test_list_repos_degrades_when_github_is_down(client, monkeypatch):
    ...
    monkeypatch.setattr("factory.execution.router._github_get", github_down)
    monkeypatch.setattr("factory.execution.router._DEFAULT_BRANCH_CACHE", {})
...
def test_list_repos_success_cache_hit_and_expiry(client, monkeypatch):
    ...
    monkeypatch.setattr("factory.execution.router._DEFAULT_BRANCH_CACHE", {})
    monkeypatch.setattr("factory.execution.router.time.monotonic", lambda: now)
    monkeypatch.setattr("factory.execution.router._github_get", github_get)
    ...
    monkeypatch.setattr("factory.execution.router._github_get", github_failed)
    second = client.get("/api/agents/repos")
    assert second.json() == first.json()
    assert len(calls) == len(repo_ids)

    monkeypatch.setattr("factory.execution.router.time.monotonic", lambda: now + 301)
    monkeypatch.setattr("factory.execution.router._github_get", github_get)
...
def test_list_repo_branches_success_with_pagination(client, monkeypatch):
    ...
    monkeypatch.setattr("factory.execution.router._BRANCH_LIST_CACHE", {})
...
def test_list_repo_branches_cache_expires_and_failures_are_shorter(client, monkeypatch):
    ...
    monkeypatch.setattr("factory.execution.router.time.monotonic", lambda: now + 11)
    recovered = client.get("/api/agents/repos/jomcgi-org/homelab/branches")
    assert recovered.status_code == 200
    assert len(calls) == 3

    monkeypatch.setattr("factory.execution.router.time.monotonic", lambda: now + 72)
...
def test_send_message_happy_path(client, session, monkeypatch):
    ...
    monkeypatch.setattr(
        "factory.execution.router.mcp.recover_zombie_session_if_needed", recover
    )
    monkeypatch.setattr("factory.execution.router._persist_pending_message", persist)
    monkeypatch.setattr(
        "factory.execution.router._activate_session_after_enqueue",
        activate,
    )
    monkeypatch.setattr(
        "factory.execution.router._set_session_status",
...
def test_send_message_gates_on_effective_model(client, session, monkeypatch):
    ...
    monkeypatch.setattr("factory.execution.router._load_session_row", lambda _: row)
    monkeypatch.setattr(
        "factory.execution.router.mcp.recover_zombie_session_if_needed",
        lambda _session_id: asyncio.sleep(0),
    )
    monkeypatch.setattr(
        "factory.execution.router._activate_session_after_enqueue", lambda _: True
    )
    monkeypatch.setattr("factory.execution.router.codex_login_gate", fake_gate)
    monkeypatch.setattr(
        "factory.execution.router._persist_pending_message",
        lambda session_id, prompt, model: (
            store.create_pending_message(session, session_id, prompt, model).seq
...
def test_router_start_codex_login_required_preserves_session_and_watches(
    ...
    monkeypatch.setattr(
        "factory.execution.router.watch_for_login",
    ...
    monkeypatch.setattr(
        "factory.execution.router._schedule_next_message", scheduled.append
    )
...
def test_prewarm_idle_bound_session_does_not_bypass_admission(
    ...
    monkeypatch.setattr(
        "factory.execution.router._transport.prewarm_session", fake_prewarm
    )
...
def test_prewarm_repeats_only_after_10s_ttl(client, session, monkeypatch):
    ...
    monkeypatch.setattr("factory.execution.router.time.monotonic", lambda: now[0])
    monkeypatch.setattr(
        "factory.execution.router._transport.prewarm_session", fake_prewarm
    )
...
def test_delete_session(client, session, monkeypatch):
    ...
    monkeypatch.setattr("factory.execution.router._load_session_row", lambda _: row)
    monkeypatch.setattr(
        "factory.execution.router._transport.destroy_session", fake_destroy
    )
    monkeypatch.setattr(
