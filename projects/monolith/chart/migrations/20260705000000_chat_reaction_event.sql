...
-- vs down on a reply) instead of only inferring from follow-up text. Reactions
-- on any bot-authored message are stored (the skill filters to ambient
-- episodes later by joining, so the write path stays simple). Only reactions
-- whose reacted message was authored by the bot are stored (target_is_bot is
-- always true here); the column is explicit to leave room to widen capture
-- later. reactor_id is the human who reacted (never the bot's own seed
-- reactions). action distinguishes add/remove so a removal cancels an earlier
-- signal.
