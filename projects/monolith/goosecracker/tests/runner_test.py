...
def test_artifact_id_is_random_stable_and_not_the_thread_id():
    """Capability URL: the artifact id is a random token stored on the thread's
    session row, reused across publishes, and never the enumerable thread id."""
    ...
    with patch("goosecracker.runner.get_engine", return_value=engine):
        first = runner._artifact_id_for_thread(thread)
        second = runner._artifact_id_for_thread(thread)
