...
async def _list_changed_files(
    ...
    Each entry is the GitHub file object carrying the path and status.
    GET /repos/{repo}/pulls/{n}/files, 100 per page, following pages until a
    short page or the page cap. Only files whose ``status`` is not ``removed``
    and whose extension is scannable are kept: a deleted file has no head
    content to fetch, and an unscannable extension has no rules.
    """
    entries: list[dict] = []
...
async def _gather_files(repo: str, pr_number: int, head_sha: str) -> list[dict]:
    ...
    content. A file whose content cannot be fetched is dropped from the scan
    input. Shares one AsyncClient across all the GitHub calls.
    """
