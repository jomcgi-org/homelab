...
loop. The Semgrep rules `no-sync-session-in-async-def` (direct Session methods
inside `async def`) and `no-session-in-to-thread` (moving an existing Session
across threads) encode this, but nothing runs them on a PR, so review has to
catch it.
...
3. The sync helper opens its **own** fresh session and commits:
   ...
   (semgrep `no-session-in-to-thread` describes why: a session is not safe to
   use across threads).
...
5. Do not `session.add` in a loop (the `session-add-in-loop` rule): build the
