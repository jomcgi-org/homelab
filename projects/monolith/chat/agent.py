...
class ChatDeps:
    ...
    # Files generated by run_code during this run, as (path, bytes) tuples.
...
def build_system_prompt(channel: str = "discord") -> str:
    ...
    if channel == "whatsapp":
        header = (
            ...
            "run_code generates (a chart, a plot, a rendered table) comes back "
    ...
    return header + (
        ...
        "- Reach for the code sandbox (run_code) by DEFAULT for anything "
        ...
        "image with run_code: the Python sandbox has a baked helper, so `from "
        ...
        "- When run_code saves a file (a chart, an image), it is attached to "
...
def create_agent(
    ...
    *,
    ...
    async def run_code(
        ctx: RunContext[ChatDeps], code: str, language: str = "python"
    ) -> str:
        ...
        and files under /tmp are lost. Returned files are attached to the reply
        automatically, so do not print paths or write markdown image links.

        Supported languages and entry points:

        - python: main.py under python3. Includes numpy, pandas, scipy,
          matplotlib (headless Agg, save figures to files), pillow, pyyaml, and
        ...
        - elixir: a script, not a Mix project, run with `elixir main.exs`.
          Elixir plus OTP only. There is no Hex, so no Jason or Nx. Do not read
          stdin.
        - ocaml: `let () = ...` entry point, run with `ocaml main.ml` in
          bytecode script mode. Standard library only. There is no opam, Core,
          or Lwt.
        - javascript: a CommonJS script under `node main.js`. Node standard
          library only. There is no npm or node_modules. `fetch` exists, but
          every call fails because there is no network.
