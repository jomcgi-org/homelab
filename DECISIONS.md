...
- NOT a hydrate problem: the zip lane bakes the unpacked, handler-imported state into the base snapshot at BuildBase (the archive is build-time-only, ADR embervm/002), so a serving cold boot of a zip base already has the handler imported. The ONLY gap was the transport.
