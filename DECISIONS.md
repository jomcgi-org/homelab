...
## D-R3.3.1: serving base-refcount guard-widening deferred to Task 9 (atomic with first reporter)
- evictable?/1 is a single shared guard gating superseded-base eviction for every workload class (task, session, serving). Adding a required serving: 0 term in Task 3, before any code reports a serving count, would make the guard fail for ALL classes (nil != 0), silently wedging base eviction repo-wide until a reporter ships.
- Task 3 therefore threads a :serving key through the counts machinery (merge_refcounts/maybe_put_count) so the mechanism recognizes serving, but does NOT widen evictable?/1. The guard stays primed/sessions-keyed.
- Task 9 MUST, in one commit, both widen evictable?/1 to require serving: 0 AND land the first ServingStore report_base_refs reporter. Widening without a reporter wedges eviction; reporting without widening lets a live serving instance's birth base evict under it. Both directions are wrong; they must land together.
- FLAG FOR JOE: confirm the deferred guard-widening is acceptable and that Task 9 carries the atomic widen+report requirement forward.
