...
- STATUS: shipped (Embervm.TcpActivator, Embervm.StatefulManager, the EndpointPublisher activator_ip fix, Embervm.Application wiring). No chart changes in this task: EMBERVM_STATEFUL_ACTIVATOR_IP / _PORT_RANGE (or _PORT_START/_PORT_END) are read but not yet rendered by chart values; with them unset the activator binds nothing (a safe no-op), exactly the "cannot wake yet" state EndpointPublisher already handles for cold stateful workloads.
