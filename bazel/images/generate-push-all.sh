...
IMAGE_TARGETS=$(echo -e "${OCI_PUSH}" | grep -v '^$' | LC_ALL=C sort)
...
#
# Its own package deliberately. validate-generate-scripts.sh diffs EVERY "//..."
# label in bazel/images/BUILD against `bazel query`, so the `.info` labels below
# would read as drift if they lived there.
#
# Derivation is pure string substitution on the same OCI_PUSH list that feeds
# push_all, so the manifest cannot cover a different set of images than the
# multirun does. Every image macro emits a `.info` sibling next to `.push`; a
# raw oci_push needs an oci_image_info added by hand, and the rule's provider
# requirement turns a missing one into an analysis error rather than a silently
# unpushable image.
DIGESTS_BUILD="bazel/images/digests/BUILD"
mkdir -p bazel/images/digests

cat >"$DIGESTS_BUILD" <<'HEADER'
# AUTO-GENERATED FILE - DO NOT EDIT (image digest manifest)
# Generated by //bazel/images:generate-push-all
# Run 'format' to regenerate

load("//bazel/tools/oci:push_manifest.bzl", "oci_push_manifest")
...
while IFS= read -r target; do
	echo "        \"${target%.push}.info\": \"$target\"," >>"$DIGESTS_BUILD"
done <<<"$IMAGE_TARGETS"

cat >>"$DIGESTS_BUILD" <<'FOOTER'
    },
    visibility = ["//visibility:public"],
)
FOOTER
