...
**Why.** Monolith package classification now lives in package-local BUILD targets. The private backend aggregates targets tagged `monolith-backend`; public packages expose separately pruned targets tagged `monolith-public` to a dedicated aggregator package, so private targets are not visible to the public tier. Existing test targets moved with their source packages, while the root binary and library labels remain stable for consumers. The central BUILD ratchet compares semantic entries instead of aggregate counts, so removing one allowed entry cannot pay for a different exclusion, glob, duplicate, replacement, or move. It covers `gazelle:exclude` directives and named-package `glob()` patterns, and has no command-line escape hatch: any exception requires a documented, narrowly scoped code change reviewed through required CI (#3930, #3931, #3932).
...
| tooling/009 | Per-package visibility and tags classify monolith packages; lint out central `gazelle:exclude` | Accepted, executed; package-owned targets and the CI ratchet landed together (#3930, #3931, #3932) | deleted |
