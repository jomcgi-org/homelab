...
Per-arch toolchains come from the `OCAML_ARCHES` registry: linux x86_64 and aarch64 are both enabled and registered, aarch64 is a registered execution platform, and there is deliberately no unconstrained fallback toolchain (it would outrank the per-arch ones from the default amd64 executor and hand aarch64 targets x86_64 binaries). Every `pr-checks` run starts with `executor_arch_probe_arm64_test`; its `uname -m` action is uncached, cannot fall back locally, prints the observed architecture, and fails unless BuildBuddy's `Arch: arm64` routing reaches an aarch64 executor. The same required action then tests `//bazel/ocaml/examples/...` with the aarch64 target platform, exercising the native aarch64 sysroot and toolchain. The `no-arm64` tags continue to exclude C-stub examples until #3923 supplies an aarch64 C/C++ toolchain; the default `//...` coverage still runs the examples on amd64. There is no OCaml Gazelle extension and no CLI release lane; both are decided and unbuilt (Decision history).
