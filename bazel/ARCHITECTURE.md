...
No Bazel runs on a Mac. `bootstrap.sh` pulls the host's slice of the tools image named in `.tools-version` (`ghcr.io/jomcgi/homelab/bazel/tools/image:main`, built by `bazel/tools/image/BUILD` as a `rules_oci` index over linux/amd64, linux/arm64 and darwin/arm64) with `crane export` into `~/.cache/homelab-tools`, keyed by manifest digest so a re-run is a no-op until the image moves. `.envrc` puts `bazel/tools/ci` and that cache's `usr/bin` on PATH and checks for a newer digest in the background. The image carries pinned per-platform binaries (argocd, bb, buildifier, buildozer, claude, crane, gh, gofumpt, helm, kind, op, ruff, shellcheck, shfmt), cross-built repository tools (agent-run, hf2oci), node, eslint, pnpm, prettier with the Svelte plugin, a Python 3.13 runtime with the CLI's pip closure, and the hand-written `homelab` CLI from `tools/cli/`. Bootstrap validates the issue #3915 commands before recording a digest, so a partial extraction is never accepted as current. The image deliberately carries no `gazelle`: the only gazelle the multitool lockfile can supply is aspect-gazelle, a different program from `//:gazelle_binary`, and the correct one links a cgo tree-sitter that the amd64 executors cannot cross-build for darwin.
...
One action in `buildbuddy.yaml`, `pr-checks`, handles pull requests, merge-queue candidates (`gh-readonly-queue/*`, which arrive as push events), and pushes to `main`. Its exact name remains the GitHub ruleset's required status check. Every run clones full history (`git_fetch_depth: 0`) and uses the same runner and snapshot namespace. A gate requires `GIT_BRANCH` to equal `GIT_REPO_DEFAULT_BRANCH` and `GIT_PR_NUMBER` to be zero before routing into publishing or chart-version write-back. The PR-number condition keeps a pull request from a fork branch named `main` out of that route: PR events set `GIT_BRANCH` to the head ref and `GIT_PR_NUMBER` non-zero, push events never set a PR number, and a fork's push fires the fork's webhooks. Credential isolation is unchanged by the gate: BuildBuddy injects secrets for every trusted event, including same-repo events and collaborator fork PRs, executes the YAML from the pushed branch, and already supplied `GHCR_TOKEN` to `pr-checks`; `ON_MAIN` is a routing guard inside a PR-controlled script.

Tests run before the main-only publish, so a failing suite cannot ship images or charts. The native arm64 OCaml shard now gates main's publish, while the old `deploy` action did not run it. PR and merge-queue runs prove the image build and render the manifest diff but never publish. A normal main run calls `push-changed.sh`, writes published chart versions back as the repository owner through the ruleset's existing bypass, then runs formatting and generators read-only. Publishing stays before formatting so a formatter tool failure cannot prevent tested images from shipping, while format drift still makes the action fail. A main push whose head is the `chore(charts): publish` write-back still runs the tests to refresh the shared snapshot but skips publishing, write-back and formatting: `push.sh.tpl` finds nothing to publish for that head and the commit touches only version lines. A push whose head is a `style: auto-format` commit publishes normally, because rebase merges routinely land the format bot's commit as the head of a PR that changed a chart. A write-back head skips the read-only format check, so generator drift from a write-back surfaces on the next author's commit. PR and merge-queue runs now have a two-hour ceiling, as do main runs, for a wide cold image rebuild.

Chart versions advance for conventional commits that change effective release inputs, not every file in a Bazel package. `chart-version.sh` queries the concrete source and build-file closure of `chart.package`, including pinned-image and shared sources, and adds the active deploy render targets plus their Application and kustomization inputs. A documentation or test file outside that closure does not publish merely because it shares a package with an input. A documentation-looking file that is packaged or otherwise selected by the graph remains an input. Deletions and renames get a conservative package-level guard because a removed file cannot appear in the current closure. A missing Bazel command, a query error, partial output, an empty closure or an unsupported label falls back to a repo-wide commit walk. This can over-publish, but cannot silently suppress a real release. Git-walk or version-computation failures stop the caller rather than reusing the current published version.
...
**Why.** Helm proves that templates render, but it does not enforce the API server's required fields or reject unknown fields. `//bazel/helm:chart_admissibility_test` validates every first-party and platform chart render, including admissible GKE value overlays, with kubeconform strict mode against the hub's pinned Kubernetes 1.36.3 schemas and CRD schemas derived from pinned charts. GKE auto-upgrades the control plane, so the pin is hand-maintained against `kubectl version` on the hub and drifts silently otherwise. The offline schemas and an invalid `secretKeyRef` negative control keep the result reproducible and fail closed without giving CI cluster credentials.
...
**Why.** BuildBuddy keys runner snapshots by action name, so the former `pr-checks` and `deploy` actions could not share state even with identical resources. A tests-only `pr-checks` seed on main warmed one namespace but duplicated the full test run and still saved a second deployment snapshot. Folding deploy into the required action makes one main test result gate publication and refreshes one snapshot for later runs. Credential isolation is unchanged by the routing gate, not preserved by it. A main publish run restores a VM snapshot last written by a main run or a merge-queue candidate because BuildBuddy keys queue runs under the default-branch key, while the universal fallback can be written by any cold branch run, so re-running tests in that VM does not vouch for its state; the trust boundary is BuildBuddy's trusted-author check plus RBE for everything except apko, and the action split never provided one. Publishing before formatting accepts an unformatted main turning the action red in exchange for never blocking a good image on a missing formatter binary. Full-history clones cost about 150 MB once per cold fetch and let `chart-version.sh` walk conventional commits since the last publish; a shallow clone computed "no bump" for every chart and took main's deploy down.

**Why.** The central monolith BUILD ratchet compares semantic entries, including their assignment or target attribute, instead of aggregate counts so removing one grandfathered entry cannot pay for a different exclusion, glob, duplicate, replacement, or move. It intentionally covers `gazelle:exclude` directives and named-package `glob()` patterns, not explicit `srcs` or other file lists: already-excluded packages still need central test targets, while a genuinely new package also needs an exclusion that the ratchet catches. It deliberately has no command-line escape hatch: any exception requires a documented, narrowly scoped code change that passes review and the required CI, keeping the unlock path visible and reviewable (#3932).
...
CI guards each have their pure logic pinned by a bazel test. `central_build_ratchet.py` runs before tests and rejects new semantic `gazelle:exclude` directives and named-package globs in `projects/monolith/BUILD`. `chart_version_guard.py` runs beside it and rejects a PR that moves a published chart's `Chart.yaml` `version:` or raises the `targetRevision:` the bot writes for it (ADR platform/009: those lines belong to `chart-version-bot` after merge). Its scope mirrors `write-back-versions.sh`, so files the bot never writes stay hand-editable, and lowering a pin stays allowed as the revert lever. `source_ratchet.py` replaces the custom Semgrep rules removed in #4777 as a ratchet: it fails a PR that adds a new hand-pinned digest on a repo-built image, an in-cluster URL in source, sync Session I/O in `async def`, a Session passed to a thread, `session.add` in a loop, or a `kubectl` write in a script, and grandfathers what already exists. A line opts out with a `ratchet-allow: <rule> (<reason>)` comment. The format stage runs `validate-generate-scripts.sh` (the grep-based generators agree with `bazel query`), `check-migration-ordering.sh` (linear, non-duplicated Atlas versions), `check-commit-msg-ascii.sh --all` (Wrangler rejects non-ASCII commit titles), `validate-hooks-executable.sh` (every hook in `.claude/settings.json` exists and is executable), `check_readme_structure.py` (root README links resolve and every `projects/*` is mentioned or allowlisted), `check_doc_links.py` (every ADR path reference resolves, over `git ls-files`, which is what makes deleting a harvested ADR safe), and `check_helm_deps.py` (vendored chart tarballs match their `file://` source and no chart pins a build-timestamped tag).
...
(see: `bazel/tools/hooks/`, `bazel/tools/git/`, `.claude/settings.json`)

**Why.** Hooks encode the invariants a reviewer would otherwise re-explain every session: `kubectl` is read-only, no local Bazel, no truncated gate output, no plans in the repo. Each blocking hook exists because the failure it prevents was observed and was silent; the advisory ones stay advisory where a false positive would block legitimate work. `validate-hooks-executable.sh` exists because a hook committed as mode 644 exits 126 on every call and nothing else notices. Claude hooks fire only for Claude, so a rule that must hold for every author (human, Codex, the factory's workers) lives in CI or a git hook, and a Claude hook is at most its fast-fail duplicate.
...
**Why.** The Bazel layer was built so scans would be hermetic and cached (ADR security/001), but its wrapper treated an engine that would not start as "wrong platform" and exited 0, so on the Linux runner every target passed without scanning. Repairing it meant republishing engine images, a fail-closed wrapper and triaging 35 targets of real findings, all to duplicate Managed Scans. The guest that also carried the custom rules was unused. Removing both trades rules that never gated anything for a smaller build graph, no weekly digest churn, and no false green.
...
Per-arch toolchains come from the `OCAML_ARCHES` registry: linux x86_64 and aarch64 are both enabled and registered, aarch64 is a registered execution platform, and there is deliberately no unconstrained fallback OCaml toolchain (it would outrank the per-arch ones from the default amd64 executor and hand aarch64 targets x86_64 binaries). The Linux platforms also declare GCC compatibility, and CI registers BuildBuddy's constrained x86_64 and aarch64 C/C++ toolchains so cc_library actions build natively on the matching pool. Every PR and merge queue run selects the aarch64 platform for the OCaml example shard without excluding C dependencies; PCRE2, tree-sitter, and a tree-sitter C++ scanner test are forced to execute with visible output. The arm64 pool is also asserted by `executor_arch_probe_arm64_test`. There is no OCaml Gazelle extension and no CLI release lane; both are decided and unbuilt (Decision history).
...
| tooling/009 | Per-package visibility and tags classify monolith packages; lint out central `gazelle:exclude` | Superseded 2026-09-14: the per-package migration and its follow-on ratchet closed not planned (#3930, #3931, #3932); the central-enumeration ratchet from PR #6030 remains the only control and grandfathers existing excludes | deleted |
...
| tooling/011 | Two Semgrep scan tiers: a warm single-file `mcp --pro` server for MCP and PR, and a scheduled interfile full scan on main | Superseded; the full-scan tier was removed with fc-invoke on 2026-07-28 and the EmberVM warm tier in #4777 | deleted |
| security/001 | Hermetic Semgrep via Bazel: vendored engines, direct `semgrep-core`, cached tests, Gazelle-generated targets | Superseded; the layer never scanned on the Linux runner and was removed in #4777 | deleted |
