...
`bazel/ocaml` is a native OCaml ruleset (`ocaml_library`, `ocaml_binary`, `ocaml_test`, `ocaml_ppx`) whose one purpose is building the Semgrep engine with Bazel on this repo's RBE. The compiler is the pinned Semgrep OCaml 5.3 fork, built from source with checksum-locked per-architecture Zig, Alpine sysroot/build utilities, static Bash, and Toybox inputs. The resulting compiler and native C/link tool closure travel to every OCaml action as one sysroot tar, so native stubs and links do not resolve executor PATH tools. The opam universe is a committed `lock.json` (url plus sha256 per package, maintained from a workstation by `update_lock.py`); each package is fetched and either translated from its own dune files by `dune2bazel.py`, which rejects loudly on anything it does not model, or built from a hand-written override. The pinned Semgrep CE tree (`semgrep_src`) is translated the same way, bottom-up, with a README table of the frontier reached so far. `@semgrep_src//:spacegrep_exe` is the native generic-text Spacegrep target and has build plus matching smoke tests in required Linux CI. The all-language `semgrep-core` and `osemgrep` closure remains distinct. tOyCaml (`examples/toycaml`) remains the small engine-shaped ruleset acceptance target.
