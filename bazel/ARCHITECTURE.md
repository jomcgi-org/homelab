...
CI scanning is Semgrep Managed Scans, configured on the Semgrep side rather than in this repo, and it runs registry rules only. Nothing in the Bazel graph scans code: the `semgrep_test`, `semgrep_target_test` and `semgrep_manifest_test` macros, the `semgrep` Gazelle language, the `argocd_app` manifest scan and the findings uploader were removed in #4777.

The rule library (`bazel/semgrep/rules/`, about 93 rules in ten language directories) has one consumer: the semgrep guest image, which bakes it under `/etc/semgrep/rules/` beside the Pro packs for the on-demand semgrep-scan tool (`bazel/semgrep/guest/`, `projects/firecracker/semgrep/`). So a rule such as `no-hardcoded-image-digest` flags a problem when an agent or a person asks the guest to scan, and never blocks a PR. `bazel/semgrep/tests/` keeps each rule's `# ruleid:` / `# ok:` fixtures as documentation; no target runs them.

The guest pins its engine and rule packs by hand (`third_party/semgrep_guest`, `third_party/semgrep_experimental`), both fetched through the `oci_archive` repository rule in `third_party/semgrep_pro`. The CI engine repositories (`third_party/semgrep`, the `semgrep_pro` engines, rule packs and SCA packs) and `.github/workflows/update-semgrep-pro.yaml`, which bumps their digests weekly, now have no consumer and are left for a follow-up removal.

(see: `bazel/semgrep/README.md`, `bazel/semgrep/rules/BUILD`, `bazel/semgrep/guest/BUILD`, `bazel/semgrep/third_party/`, `.semgrepignore`)

**Why.** The Bazel layer was built to make scans hermetic and cached (ADR security/001): direct `semgrep-core` cut a diff scan from over two minutes to about thirty seconds. In practice it never gated anything. The vendored engine could not load its shared libraries on the Linux runner, and the wrapper treated an engine that would not start as "wrong platform" and exited 0, so every target passed without scanning (#4777). Fixing it meant republishing engine images, a fail-closed wrapper, and triaging 35 targets of real findings, all to duplicate what Managed Scans already runs. Removing the layer trades the custom rules as a PR gate, which they never were in practice, for a smaller build graph and no false green.
