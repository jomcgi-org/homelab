...
One action in `buildbuddy.yaml`, `pr-checks`, handles pull requests, merge-queue candidates (`gh-readonly-queue/*`, which arrive as push events), and pushes to `main`. Its exact name remains the GitHub ruleset's required status check. Every run clones full history (`git_fetch_depth: 0`) and uses the same runner and snapshot namespace. A gate requires `GIT_BRANCH` to equal `GIT_REPO_DEFAULT_BRANCH` and `GIT_PR_NUMBER` to be zero before any publishing or chart-version write-back can run. The PR-number condition prevents a pull request from a fork branch named `main` from gaining publishing authority.

Tests run before the main-only publish, so a failing suite cannot ship images or charts. PR and merge-queue runs prove the image build and render the manifest diff but never publish. A normal main run calls `push-changed.sh`, writes published chart versions back as the repository owner through the ruleset's existing bypass, then runs formatting and generators read-only. Publishing stays before formatting so a formatter tool failure cannot prevent tested images from shipping, while format drift still makes the action fail. Main commits beginning with `style: auto-format` or `chore(charts): publish` still run the tests to refresh the shared snapshot, but skip publishing, write-back, and formatting to avoid loops and duplicate work. The action has a two-hour timeout for a wide cold image rebuild.
...
Merge mechanics have not changed. The repository remains rebase-only, the merge queue remains the authoritative pre-merge gate, and a PR is never rebased by hand merely for BEHIND. The details are in the `pr-workflow` skill.
