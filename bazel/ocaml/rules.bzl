...
def _driver_args(ctx, tc, mode, include_dirs, opam_pkgs, srcs, c_srcs, cc = None):
    ...
    if getattr(ctx.attr, "preprocess", None):
        ...
        # binary/test/ppx rules, which have no preprocess_data attr.
...
def _ocaml_library_impl(ctx):
    ...
    # default remote allocation and action timeout. Reserve eight BuildBuddy
    # compute units and 15 minutes for Menhir libraries, which covers both
    # generation and native compilation.
    execution_requirements = {}
    if ctx.attr.menhir:
        execution_requirements["EstimatedComputeUnits"] = "8"
...
_COMMON_ATTRS = {
    ...
    "preprocess_data": attr.label_list(
        allow_files = True,
        doc = "Files the `preprocess` ppx driver reads at preprocess time (dune " +
              "`(preprocessor_deps (file X) ...)`). Staged by basename into the work " +
              "dir next to the sources so a rewriter like ppx_blob (`[%blob \"X\"]`, " +
              "which embeds X's contents as a compile-time string) finds them. Only " +
              "meaningful when `preprocess` is set.",
    ),
