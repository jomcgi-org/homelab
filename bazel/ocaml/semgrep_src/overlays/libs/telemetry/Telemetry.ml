(* OVERLAY (bazel/ocaml/semgrep_src/overlays) -- replaces the upstream file.
   Identical to upstream except for setup_otel's backend creation: the two
   exporter clients (opentelemetry-client-ocurl: curl C bindings;
   opentelemetry-client-cohttp-eio: a full cohttp + TLS stack) are outside
   the translated closure (see semgrep_src/README.md, libs/telemetry
   dispatch). Everything else -- the real Tracing/Ometrics/Logging modules,
   the otel core, the ambient-context storage providers -- compiles for
   real; only actually exporting traces fails, loudly, at the point a
   backend would be created. Deleted when an exporter client joins the
   lock. *)
...
let setup_otel ?eio_sw_base trace_endpoint =
  ...
  let otel_backend =
    (* OVERLAY: the exporter clients are not in the translated closure;
       reject loudly where upstream would create a backend. *)
    match eio_sw_base with
    | None ->
        failwith
          ("Tracing backend not built: opentelemetry-client-ocurl is \
            outside the translated closure (semgrep_src/README.md); cannot \
            export to " ^ url)
    | Some (_sw, _base) ->
        failwith
          ("Tracing backend not built: opentelemetry-client-cohttp-eio is \
            outside the translated closure (semgrep_src/README.md); cannot \
            export to " ^ url)
  in
