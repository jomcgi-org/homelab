(* OVERLAY (bazel/ocaml/semgrep_src/overlays) -- replaces the upstream file.
   Identical to the pinned source up to the unit-tests banner; the trailing
   let%test blocks (and the s/converters/equate_positions helpers only they
   use) are stripped: ppx_inline_test is deliberately outside the Jane
   Street shallow half (decided in the plan), and without its rewriter the
   let%test syntax does not parse. *)
