(* OVERLAY (bazel/ocaml/semgrep_src/overlays) -- replaces the upstream file.
   Identical to the pinned source except the ocaml-git type equations; see
   the Git_wrapper.ml overlay for the rationale. *)
...
(* OVERLAY: value/commit/author/blob are upstream ocaml-git types
   (hash Git.Value.t &c). ocaml-git is not built; commit/author keep
   ocaml-git 3.x's record shapes, blob is the raw contents, and value is
   opaque (nothing can construct one without the object store). *)
type value [@@deriving show, eq, ord]
