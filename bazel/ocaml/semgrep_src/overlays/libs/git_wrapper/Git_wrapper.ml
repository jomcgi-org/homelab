(* OVERLAY (bazel/ocaml/semgrep_src/overlays) -- replaces the upstream file.
   Identical to the pinned source except the ocaml-git surface: the functor
   types become concrete digestif-backed equivalents (hash keeps upstream's
   own equation), commit/author mirror ocaml-git 3.x's record shapes, and
   the in-memory object-store walkers fail loudly instead of parsing git
   objects. The git-CLI shell-out layer (the bulk of this module, and all
   src/target reaches) is untouched. See semgrep_src/README.md. *)
...
(* Standard git uses sha1.
   OVERLAY: upstream builds these types with ocaml-git functors
   (Git.Hash.Make (Digestif.SHA1), Git.Value/Commit/Blob.Make, Git.User).
   ocaml-git is not in the lock (out of scope; see
   bazel/ocaml/semgrep_src/README.md), so hash keeps its upstream equation
   (Digestif.SHA1.t, digestif is locked) and commit/author mirror
   ocaml-git 3.x's record shapes field for field. value is a placeholder
   nobody constructs: the object-store walkers below fail loudly. *)

type hash = Digestif.SHA1.t

let equal_hash = Digestif.SHA1.equal
let compare_hash = Digestif.SHA1.unsafe_compare
let pp_hash fmt h = Format.pp_print_string fmt (Digestif.SHA1.to_hex h)
let show_hash = Digestif.SHA1.to_hex

type tz_offset = { sign : [ `Plus | `Minus ]; hours : int; minutes : int }
[@@deriving show, eq, ord]

type author = { name : string; email : string; date : int64 * tz_offset option }
[@@deriving show, eq, ord]
...
type blob = string [@@deriving show, eq, ord]
type value = Value_requires_ocaml_git [@@deriving show, eq, ord]
...
let commit_digest _ =
  failwith
    "Git_wrapper.commit_digest needs the ocaml-git object store, which is \
     not built (see bazel/ocaml/semgrep_src/README.md)"

let commit_author (c : commit) = c.author
let hex_of_hash = Digestif.SHA1.to_hex

(* A blob's object id is the SHA1 of "blob <decimal length>\000<contents>". *)
let blob_digest (b : blob) =
  Digestif.SHA1.digest_string (Printf.sprintf "blob %d\000%s" (String.length b) b)

let string_of_blob (b : blob) = b
...
(* OVERLAY: the in-memory object-store walkers (tree_of_commit,
   blobs_of_tree, blobs_of_entry, blobs_by_commit) need ocaml-git's value
   parsing; nothing translated reaches them, so the one exported entry
   point below fails loudly (the Telemetry.ml dispatch). *)
...
let commit_blobs_by_date _objects =
  failwith
    "Git_wrapper.commit_blobs_by_date needs the ocaml-git object store, \
     which is not built (see bazel/ocaml/semgrep_src/README.md)"
...
let cat_file_blob ?cwd (hash : hash) =
  let cmd = (git, cd cwd @ [ "cat-file"; "blob"; hex_of_hash hash ]) in
