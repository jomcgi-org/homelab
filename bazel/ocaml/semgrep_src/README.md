...
| `languages/yaml/ast`           | `:parser_yaml_ast`         | translated as-is (Phase 9 wave 2; parser_yaml.ast); names commons/lib_parsing + `(pps ppx_deriving.show)`, all resolved                                                                                                                                                                          |
| `languages/yaml/parser`        | `:parser_yaml_parser`      | translated as-is (Phase 9 wave 2; parser_yaml.parser); the first yaml-lock consumer outside src/core (names `yaml` directly) + commons/lib_parsing/ast_generic/parser_yaml.ast. Validates the ctypes-stubgen yaml override from a second caller                                                  |
| `languages/yaml/generic`       | `:parser_yaml_ast_generic` | translated as-is (Phase 9 wave 2; parser_yaml.ast_generic); names yaml + commons/lib_parsing/ast_generic/parser_yaml.ast/parser_yaml.parser. yaml CST -> AST_generic glue                                                                                                                          |
| `libs/ograph`                  | `:ograph`                  | translated as-is (Phase 9 wave 3); the deprecated object-graph lib (`public_name ograph`, commons + collections, pure, no pps), landed ahead of src/il which names it                                                                                                                              |
| `src/il`                       | `:semgrep_core_il`         | translated as-is (Phase 9 wave 3; semgrep.il); names commons/ograph/lib_parsing/ast_generic with a `(pps ppx_deriving.show/.eq/.ord ppx_hash visitors.ppx)` line. First consumer of ppx_deriving's `.eq`/`.ord` plugins (only `.show` exercised before); all locked, visitors already drives ast_generic |
| `src/analyzing`                | `:pfff_lang_GENERIC_analyze` | translated as-is (Phase 9 wave 3; pfff-lang_GENERIC-analyze); names commons/ast_generic/parallelism/semgrep.core/semgrep.il + `(pps ppx_profiling ppx_deriving.show commons.ppx telemetry.ppx)`, all resolved once src/il lands                                                                   |
| `src/prefiltering`             | `:prefiltering`            | translated as-is (Phase 9 wave 4; semgrep.prefiltering); names ast_generic/semgrep.core/semgrep.target + `(pps ppx_deriving.show/.eq/.ord ppx_hash ppx_profiling ppx_sexp_conv)`, all resolved. Two atdgen `(rule)` pairs over Semgrep_prefilter.atd (`-j -j-strict-fields` / `-t`), the genrule shape src/configuring set; atdgen-runtime reaches it transitively via semgrep.core. No new externals |
| `src/targeting`                | `:semgrep_targeting`       | translated as-is (Phase 9 wave 5; semgrep.targeting); names commons/fpath/glob/gitignore/git_wrapper/semgrep.target/semgrep.core with a `(pps ppx_blob ppx_profiling telemetry.ppx ppx_deriving.show ppx_deriving_yojson ppx_deriving.eq ppx_hash commons.ppx)` line. The first NEW external since yaml: `ppx_blob` (a clean ppx_rewriter translate, no override). The first `(preprocessor_deps (file default.semgrepignore))` consumer, handled by the new translator preprocess_data feature (dispatch below). Reaches commons' pcre, so it rides `ladder_builds_cc` |
| `src/naming`                   | `:pfff_lang_GENERIC_naming` | translated as-is (Phase 9 wave 6; pfff-lang_GENERIC-naming); names commons/ast_generic/semgrep.core/semgrep.typing/parser_javascript.ast + `(pps ppx_profiling ppx_deriving.show)`, all resolved. Reaches commons' pcre, so `ladder_builds_cc`. No new externals |
| `languages/jsonnet/ast`        | `:parser_jsonnet_ast`      | translated as-is (Phase 9 wave 7; parser_jsonnet.ast); names commons/lib_parsing + `(pps ppx_deriving.show)`, all resolved                                                                                                                                                                          |
...
| `languages/*`                  | `:parser_*`                | complete all-language parser matrix named by `semgrep.parsing`; every ocaml-tree-sitter binding is checksum-locked to the matching submodule commit at `SEMGREP_COMMIT` |
...
## languages/jsonnet rejection dispatch (ast, tree-sitter, generic) + the jsonnet grammar stamp

The second per-language tree-sitter chain (Phase 9 wave 7), plus the
libs/ojsonnet consumer that motivates it. All four dirs translate as-is (no
overlays); the one new external is the grammar:
...
| `semgrep-jsonnet/` (submodule dir under tree-sitter)               | inert: the shallow clone does not init submodules; the grammar rides the lock and the non-recursive glob never descends                                                                                                                                                                            |

The grammar stamp itself (`opam/overrides/tree-sitter-jsonnet/BUILD.tpl`):
a `jsonnet_grammar` cc_library over the generated `lib/parser.c` + the
external `lib/scanner.c` (+ the repo-local `lib/tree_sitter/*.h`, `includes =
["lib"]`) against the vendored `@ocaml_tree_sitter_c//:tree_sitter` runtime,
...
| `pfff_lang_GENERIC_analyze` (src/analyzing)                                            | internal, new dir (LANDED, wave 3): names `semgrep.il`. src/il decomposed to commons/ograph/lib_parsing/ast_generic + pps ppx_deriving.show/.eq/.ord + ppx_hash + visitors.ppx; the one new internal dep was `ograph` (libs/ograph, commons + collections, pure), landed first. No new lock entries                                                                                                                                                                                                                      |
| `semgrep.prefiltering` (src/prefiltering)                                              | internal, new dir (LANDED, wave 4): two atdgen rule pairs over Semgrep_prefilter.atd (`-j -j-strict-fields` / `-t`), the src/configuring genrule shape; deps ast_generic/semgrep.core/semgrep.target all resolved. No new externals                                                                                                                                                                                                                                  |
...
src/targeting was the first new external since the yaml closure and the first
`(preprocessor_deps ...)` consumer. Its `(libraries ...)` (commons, fpath,
glob, gitignore, git_wrapper, semgrep.target, semgrep.core) all resolved
already; the slice landed one lock entry and one translator feature:

| piece                                          | dispatch                                                                                                                                                                                                                                                                                          |
| ---------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `ppx_blob` (pps)                               | lock: ppx_blob 0.9.0 (johnwhitington/ppx_blob, the opam release tarball). Its `src/dune` is `(kind ppx_rewriter) (libraries ppxlib)` with no codegen, so dune2bazel translates it as-is (NO override, unlike ppx_deriving). Depends only on ppxlib, already locked. No `ppx_runtime` entry: `[%blob ...]` inlines a string literal, so rewritten code links nothing extra |
| `(preprocessor_deps (file default.semgrepignore))` | translator feature: dune2bazel parses `(file X)` entries into a `preprocess_data` attr (file paths src_dir-prefixed, the srcs convention); the rule stages each file by basename into the ppx work dir AND lists it as an action input; the driver passes them as `--ppx-data`. ppx_blob resolves `[%blob "X"]` relative to the source file's directory first (its `pos_fname` dirname, = the work dir, where the driver runs `$PPX --impl $WORK/foo.ml`), so the staged file is found at preprocess time with no cwd dance. `(glob_files ...)`, bare atoms, dune variables, and parent-dir escapes reject loudly (a missing blob is a build error, never a silent no-op) |
| `default.semgrepignore`                        | a regular file in upstream `src/targeting/` (not a submodule), so it rides the shallow clone automatically; the generated `preprocess_data = ["src/targeting/default.semgrepignore"]` references it as a source label in the @semgrep_src root package |
| `commons`, `fpath`, `glob`, `gitignore`, `git_wrapper`, `semgrep.target`, `semgrep.core` | in the lock / internal, all translated by wave 4                                                                                                                                                                                                                                                  |
| `(wrapped false)`                              | modeled (the Semgrep house style; the generated library omits `wrapped = True`)                                                                                                                                                                                                                   |

The feature was exercised in isolation by `examples/ppx_blob` (a tiny
`[%blob "secret.txt"]` test wired through `preprocess_data`, asserting the
embedded contents equal the file) before src/targeting consumed it.
