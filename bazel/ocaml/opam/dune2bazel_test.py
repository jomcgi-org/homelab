...
class TestGenExecutable:
    def _stanza(self, text):
        return parse(tokenize(text))[0]

    def test_public_name_becomes_target_and_dependencies_resolve(self):
        stanza = self._stanza(
            "(executable (name Main) (public_name tool) "
            "(package pkg) (libraries unix re))"
        )
        out = gen_executable(stanza, "src/bin", LIB_MAP)
        assert "ocaml_binary(" in out
        assert 'name = "tool_exe"' in out
        assert 'deps = ["@ocaml_re//:re"]' in out
        assert 'opam_deps = ["unix"]' in out
        assert '"src/bin/*.ml"' in out

    def test_internal_name_used_without_public_name(self):
        stanza = self._stanza("(executable (name Main))")
        out = gen_executable(stanza, "src", LIB_MAP)
        assert 'name = "Main"' in out

    def test_unknown_field_exits(self):
        stanza = self._stanza("(executable (name Main) (modes native byte))")
        with pytest.raises(SystemExit):
            gen_executable(stanza, "src", LIB_MAP)

    def test_public_install_path_exits(self):
        stanza = self._stanza("(executable (name Main) (public_name bin/tool))")
        with pytest.raises(SystemExit):
            gen_executable(stanza, "src", LIB_MAP)
...
class TestGenDuneDir:
    ...
    def test_executable_stanza_translates(self, tmp_path):
        path = self._write(tmp_path, "(executable (name main))\n")
        (tmp_path / "src" / "main.ml").write_text("let () = ()\n")
        out = gen_dune_dir(path, "src", LIB_MAP)
        assert "ocaml_binary(" in out
        assert 'name = "main"' in out

    def test_executable_without_main_module_exits(self, tmp_path):
        path = self._write(tmp_path, "(executable (name main))\n")
        with pytest.raises(SystemExit):
            gen_dune_dir(path, "src", LIB_MAP)

    def test_plural_executables_stanza_exits(self, tmp_path):
        path = self._write(tmp_path, "(executables (names one two))\n")
        with pytest.raises(SystemExit):
            gen_dune_dir(path, "src", LIB_MAP)

    def test_no_build_stanza_exits(self, tmp_path):
    ...
    def test_menhir_stanza_attaches_to_library(self, tmp_path):
        ...
        assert '"EstimatedComputeUnits": "2"' in out
...
class TestMain:
    ...
    def test_reads_dune_file_and_emits_build(self, tmp_path, capsys, monkeypatch):
        ...
        assert (
            'load("@my_root//bazel/ocaml:defs.bzl", "ocaml_binary", "ocaml_library", "ocaml_ppx")'
            in captured.out
        )
