...
| `argocd_app`           | macro | `app.bzl`    | Declare a deploy overlay: template test, pre-rendered manifests                         |
...
Declares an ArgoCD overlay. Always emits a `template_test`; opt into
pre-rendered `render_manifests` and a live `diff`.
...
write them only when you need attributes Gazelle does not author.
