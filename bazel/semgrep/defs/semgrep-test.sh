...
# Env: SEMGREP_TEST_MODE, if "1", fails because annotation checks are unavailable
...
# Probe both staged engines. The Pro probe catches loader and dependency errors
# before they can be mistaken for a clean scan.
...
# semgrep-core cannot verify the annotations, so reporting success is unsafe.
if [[ "${SEMGREP_TEST_MODE:-}" == "1" ]]; then
	echo "ERROR: SEMGREP_TEST_MODE=1 requires pysemgrep and cannot run this scan" >&2
	exit 2
fi
...
if [[ ${#_ALL_LANGS[@]} -gt 0 ]]; then
	LANGS_FILE="${TEST_TMPDIR}/scan-languages.txt"
	printf '%s\n' "${_ALL_LANGS[@]}" | sort -u >"$LANGS_FILE"
...
for lang in ${SCAN_LANGS[@]+"${SCAN_LANGS[@]}"}; do
	for rule_file in "${RULE_FILES[@]}"; do
		...
		if [[ "$SCAN_EXIT_CUR" -ne 0 ]]; then
			echo "ERROR: semgrep-core exited $SCAN_EXIT_CUR on $(basename "$rule_file") (lang=$lang)" >&2
			cat "$STDERR_FILE" >&2
			exit 2
		fi
...
if [[ ${#LOCKFILE_FILES[@]} -gt 0 ]]; then
	...
	for rule_file in "${RULE_FILES[@]}"; do
		...
		if [[ "$SCAN_EXIT_CUR" -ne 0 ]]; then
			echo "ERROR: semgrep-core (SCA) exited $SCAN_EXIT_CUR on $(basename "$rule_file")" >&2
			cat "$STDERR_FILE" >&2
			exit 2
		fi
...
if [[ "$SCAN_EXIT" -eq 1 && "$HAS_LOCKFILES" == "true" ]]; then
	LOCKFILE_PATHS=""
	for lf in "${LOCKFILE_FILES[@]}"; do
		LOCKFILE_PATHS+="$lf"$'\n'
	done
