...
1. **Apache Iceberg on SeaweedFS** as the **single source of truth** for all append-only domains: notes (every revision), embeddings, ingest events, job-run logs, gardener output, knowledge raws. Append-only by convention — mutations become supersession events, deletes become tombstones.
...
| ----------------------- | ------------------------------------------ | ----------------------------------------------------------------------- |
| **Source of truth**     | CNPG Postgres (pgvector + notes tables)    | Iceberg on SeaweedFS (append-only event log)                            |
...
| **Backup granularity**  | One cluster, PITR for everything           | PITR only for operational PG; Iceberg gets snapshot/versioning natively |
...
graph TB
    subgraph "Write Path"
        Q[Scheduler queue<br/>operational PG]
        Q -->|claim job| W[Ingest workers]
        W -->|append event| ICE
    end

    subgraph "Source of Truth"
        ICE[(Iceberg tables<br/>on SeaweedFS<br/>append-only)]
    end

    subgraph "Build Path"
        CRON[15min builder job<br/>scheduler-driven]
...
Coalescing knob: chart bumps should not fire every 15min (would spam PRs and constantly churn the serving layer). The Image Updater config sets a minimum interval (e.g. 1h) between chart bumps; the OCI artifact still updates every 15min, but pods only roll hourly. Clients pulling stale data within that window is acceptable per the consistency model.

---

## Implementation

### Phase 1: Foundation — SeaweedFS + Iceberg catalog

- [ ] Deploy SeaweedFS to the cluster with appropriate replication mode (`010` rack-aware) for the warehouse bucket
- [ ] Choose and deploy an Iceberg REST catalog (default: Nessie, with catalog state in the operational PG)
- [ ] Create the `notes_warehouse` Iceberg namespace with table schemas for `note_events`, `ingest_events`, `job_runs`
- [ ] Write a thin Python/Go writer library used by ingest workers to append events
- [ ] Document the event schemas in `docs/observability.md` style

### Phase 2: Dual-write & backfill

- [ ] Modify the ingest path to dual-write: continue writing to PG, also append to Iceberg
- [ ] One-shot backfill job: read all existing notes from PG, write a synthetic `created` event per note + a `current` version event with the latest embedding into Iceberg
- [ ] Validate row counts and sample queries match between PG and Iceberg

### Phase 3: Builder pipeline

- [ ] Deploy zot in-cluster, backed by SeaweedFS S3
- [ ] Register `rebuild-notes-duckdb` as a scheduler routine job (15min cadence)
- [ ] Builder job: read Iceberg active-period events → build `notes-active.duckdb` with per-partition HNSW → upload as OCI layer → push tag bump
...
- [ ] Add runbooks for: failed monthly compaction, OCI registry full, Quack server OOM, builder job stuck
- [ ] Add SigNoz dashboards for builder duration, artifact size growth, query fan-out distribution
...
- **Nessie catalog** is internal-only. Auth via the operational PG (existing trust boundary).
...
| Iceberg catalog (Nessie) becomes a SPoF                       | Low        | High   | Catalog state lives in operational PG (already PITR-backed); Nessie deployment runs ≥2 replicas                                             |
...
1. **Nessie vs Polaris vs lakekeeper for the catalog.** Default to Nessie (most mature OSS, simplest single-binary). Revisit if we need multi-tenant catalog isolation or git-style branching at the catalog level.
...
6. **Whether to keep pgvector around as a sanity-check shadow query for some period after cutover.** Cheap insurance, costs a PG column we'd otherwise drop.
...
| [Nessie — transactional catalog for Iceberg](https://projectnessie.org/)                                              | Candidate catalog implementation                                         |
