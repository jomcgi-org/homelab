...
Weight rather than `cpu.max` because under decision 5 the sum of entitlements deliberately exceeds hardware, so a quota cannot guarantee its number anyway, and a hard ceiling would idle cores while VMs sit frozen. Weight degrades to proportional share under contention and lets a workload burst into idle capacity.
