...
Both are RPO 0, because a planned drain always takes a final archive. **The configured `archiveInterval` bounds loss only for *involuntary* node loss**, which is the genuinely unplanned case.

**The continuity contract mirrors ADR 016's session guarantee:** a stateful workload gets a guaranteed window of continuous uptime (8h, matching the session ceiling), after which it may be moved with bounded disruption. That is a promise a developer can plan against, and it makes rotation a stated property rather than an incident.
