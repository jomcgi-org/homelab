...
- **[ADR 025](025-local-disk-authoritative-s3-archive-interval.md) removes one of the physical fences this ADR leans on.** The Longhorn attach-exclusivity tier no longer applies to stateful volumes: they stay on local node NVMe, so the fence is the single writable attach in `volume.Manager` plus the volume's node-locality, with no block-layer attach arbiter behind it. The partition bound for a volume that is still attached on a silent brick is therefore the brick silence timeout ([ADR 023](023-class-scoped-ownership-arbitration.md) decision 3b), which is why 023 extends that timeout to stateful rather than leaving the two-writer window unbounded after a deliberate restore-elsewhere.
