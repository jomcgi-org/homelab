...
| [037](037-brick-silence-timeout.md) | Brick silence timeout: noded bounds its own node-local authority by wall-clock silence, tracking last control-plane contact from the only two live channels (dial-home Register 2xx and WatchNode traffic) and standing down when quiet too long, the wall-clock complement to ADR 023's generation-bounded leases; implementation tracked in #5073 | Draft |
| [038](038-image-lane-serving-fresh-boot.md) | Image-lane serving fresh boot: a `serving: true` BuildBase with no archive registers its built rootfs itself as the serving-images inventory entry (empty handler_path), and fresh boot boots it without a handler drive, closing the zip-lane-only gap D-R3.11.2 deferred; carries the 2026-08-23 correction that the image-lane clause never stamped `serving:` so every image-lane wake failed | Draft |
| [039](039-capacity-fair-share-blast-radius-signal.md) | Capacity redesign from the 2026-09-01 GKE incidents: brick CPU requests become CFS fair-share weights (no CPU limit), memory requests move to ~75% of limit (limits keep the guest-admission ceiling); overcommit blast radius is a ladder (guest jailer cgroup, then brick eviction, then node); pressure sheds LRU idle guests before eviction; `homelab-preemptible` (-9) makes bricks autoscaler-visible while staying shed-first; unsatisfiable demand is a signaled capacity condition, not a retry loop. Control-plane-driven class provisioning, in-place resize, and the scratch-prep boot-order race are deferred | Accepted |
| [040](040-anchor-loss-recovery-preemption-budget-realism.md) | Amends standing decision 11 after GKE spot churn left `demo-postgres` down twice in 18h despite an intact export: an exported volume may now restore onto a live peer once its anchor is confirmed gone (absent from capacity for the full 90s registry-expiry window, with the control plane itself up that long); the restore writes only `node_id`, so the control plane stays sole generation issuer and a stale returning copy fails its pair check. Standing decision 7 gets one exception: a stateful-volume export may hold the drain open within the remaining budget. `DrainTimeout`'s 110s default is flagged as the AWS spot contract rather than GCE's actual ~30s notice, pending a metadata-server watcher and a graceful/preemption budget split. `anchorLossPolicy` and NAS-backed volumes are named and deliberately deferred | Accepted |
| [041](041-spiffe-workload-identity-self-hosted-spire.md) | Self-hosted SPIRE closes three identity gaps with one issuer: guests get per-principal JWT-SVIDs delivered over vsock by noded as a SPIRE delegate (superseding ADR 024 decision 3's Kubernetes-TokenRequest mechanism, not its identity-not-capability policy); platform components get X.509-SVID mTLS, replacing the static CP-to-noded bearer `node.proto` reserved the upgrade for; the egress proxy authorizes by `(principal, host)` instead of host alone, closing #5320 structurally. GCP workload identity federation follows in phase 5. Resolves the SPIFFE deferrals in ADR platform/012, networking/003, embervm/032, and embervm/022 for EmberVM specifically; phase plan tracked in #5706 | Accepted |
