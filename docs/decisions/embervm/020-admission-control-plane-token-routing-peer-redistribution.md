...
Ownership is settled in [ADR 023](023-class-scoped-ownership-arbitration.md), which makes arbitration class-scoped: stateful keeps the existing physical fence unchanged, and the session class accepts bounded divergence with a durable relinquish record as the handoff commit point. Decisions 2, 4 and 5 take their staleness semantics from there, and decision 5's claim below is corrected by it.
