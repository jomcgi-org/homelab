...
| R7 Distribution | Redistribute and pre-warm workloads across many nodes | Serving and task fleets on a multi-node Firecracker pool | Placement policy, demand-driven artifact warming, multi-node endpoint fan-out | Placement moves are a copy, never a rebuild; bounded only by ISA/CPU compatibility and volume anchoring | Decided ([009](009-roadmap-extension-continuity-before-tenancy.md), amended) |
