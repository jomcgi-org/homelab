...
**Builds on:** [019 - Substrate Executor + AgentWorkflow over Argo](019-substrate-executor-agentworkflow.md) (consumes its `AgentWorkflow` tier and `Snapshotable` executor), [003 - Context Forge](003-context-forge.md) / [020 - Deprecate Context Forge](020-deprecate-context-forge-mcp-gateway.md) (MCP surface), [security/003 - gVisor RuntimeClass](../security/003-gvisor-runtime-class.md) (isolation prerequisite)
...
- **Trust axis unchanged from 019.** Harnesses are ours today, so cold-on-demand / warm pool suffice and no VM boundary is required. When untrusted input or external callers arrive, isolation flips to mandatory: gVisor (`runsc`, [security/003](../security/003-gvisor-runtime-class.md)) and/or `runtimeClassName: kata-fc` (Firecracker microVM), absorbed additively behind the `Substrate` seam without touching this consumer.
