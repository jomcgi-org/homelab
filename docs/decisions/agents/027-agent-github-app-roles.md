...
- **Require code-owner review (`@jomcgi`) on every PR.** Rejected for the autonomous loop: it would put the human in the inner loop on every change, defeating the point. `@jomcgi` stays an owner who _can_ act, gated by the reviewer check for the automated path. A path-scoped second `CODEOWNERS` rule could still force human review on genuinely sensitive directories later (see Open Questions).
...
| Resource                                                                                                        | Relevance                                                               |
| --------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------- |
| ADR 023 - Egress Secret Proxy                                                     | The placeholder-swap mechanism that injects each role token at egress.  |
| ADR 024 - Discord Agent, Hosted-Model Tiers, Artifacts | The single `gh` token coding tier this ADR splits into two role tokens. |
| ADR 025 - Three-Layer Agent Stack                                | goosecracker as the per-thread config surface that selects the role.    |
