...
Expected: columns `session_id`, `recipe`, `tier`, `state`, `task`, `created_at` all exist on the agent_threads model. If any name differs, fix `_CONTEXT_SQL` to match (improve-recipes' `_GATHER_SQL` is the authority; it selects the same columns).
