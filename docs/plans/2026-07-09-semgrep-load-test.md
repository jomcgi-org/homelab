...
**Global one-run-at-a-time guard + agent-idle check:** at most one load test runs at any moment across ALL workloads, AND a load test refuses to start while an agent (goose) run is active. This keeps the cgroup-memory assumption honest: the 26Gi daemon limit is sized for one load-test workload at its cap (~22.5Gi) plus daemon overhead, NOT for coexistence with the 8Gi agent workload. Enforce in the create endpoint: (a) if any `load_run` row is `running`, return it instead of starting a new one; (b) if any agent run is currently active (check the goosecracker threads ledger / `claude_agent.agent_threads` for a non-terminal state), refuse with a clear message. The agent workload arrives asynchronously from Discord/MCP so this only blocks the START of a load test; a run landing mid-test is the rare residual that oom_score_adj (guest dies first) backstops.
