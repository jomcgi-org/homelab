...
- World Atlas 2015: `https://datapub.gfz-potsdam.de/download/10.5880.GFZ.1.4.2016.001/World_Atlas_2015.zip` → `World_Atlas_2015.tif` (EPSG:4326, mcd/m², **lower = darker**; mask sea/nodata before percentiling).
- Roads: `https://download.geofabrik.de/europe/united-kingdom/scotland-latest-free.shp.zip` → `gis_osm_roads_free_1.shp`; distance test in EPSG:27700 via `STRtree.query(predicate="dwithin", distance=2000)`.
