...
2. **Manifest is regenerated manually** via `bazel run //projects/monolith:gen_repo_docs_manifest`, then committed. It was originally intended to live in the `//bazel/tools/format:format` multirun for auto-regen, but that aggregate drives only sh_binary generators and a py_venv_binary does not resolve its main module under rules_multirun, so it is kept out of the aggregate. A stale manifest is low-risk: the reconcile job is idempotent and only delays indexing of newly changed docs until the next regeneration. Wiring auto-regen via a hermetic interpreter is a follow-up.
