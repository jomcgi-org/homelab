...
**Goal:** Implement ADR embervm/014: make node-agent dial-home reports the source of truth for instance runtime state, take durable writes off the boot/wake hot path, make placement reject/retry, pre-provision taps at brick boot, make destruction node-confirmed, and add the `isolated_execution` workload contract.

**Architecture:** Six PRs, ordered so each is independently shippable and gated. PR 1 (isolated_execution) and PR 2 (node-confirmed destruction) are pure safety wins with no latency dependency. PR 3 (reject/retry placement) and PR 4 (async lifecycle writes) deliver the hot-path latency. PR 5 (tap pre-provisioning) removes netlink work from instance boot. PR 6 is the mandatory TLA+ follow-through (ADR 014 requires every implementation plan to carry this explicitly). Every behavioural change ships behind an off-by-default env gate following the `EMBERVM_WARMTH_RETENTION_SWEEP` pattern, flipped in a later values-only commit once proven.

**Tech Stack:** Elixir control plane (`projects/embervm/control`), Go node agent (`projects/embervm/noded`), protobuf (`projects/embervm/proto/embervm/node/v1/node.proto`), Workload CRD (`projects/embervm/chart/crds/workload-crd.yaml`), TLA+ specs (`projects/embervm/specs/`), Helm chart + ArgoCD deploy.
...
| ----------------- | --------------------- | --------- |
| Q1: worker authority for serving/xDS relay state | **Out of scope.** Serving relay reconciliation stays as-is. | ServingSweeper already treats node Envoy stats as truth; extending authority to xDS routes is an R3 concern with its own failure modes. Revisit after PR 2 lands. |
...
| Q4: `isolated_execution` default | **Default false (opt-in).** | Every existing lane (warm pool, R2 bank/relight, serving/stateful banking) depends on reuse; defaulting true would silently forfeit warm starts fleet-wide. Scanning/multi-tenant workloads set it explicitly. |
| Q5: does the flag forbid booting FROM shared warm snapshots | **Capture-only** (the ADR's own current answer). Booting from a pre-workload base snapshot is allowed; capture, pool return, relight, and bank of the instance's own state are forbidden. | Base snapshots contain nothing tenant-owned. A future `distrust_shared_base` refinement can come later if a scanning lane demands it. |

## What is already ADR-014-shaped (do not rebuild)

- **Adoption reconcile** already treats node reports as truth for sessions, serving, stateful, and group state (`session_manager.ex do_reconcile/1`, sweepers, `stateful_manager.ex reconcile/1`). PR 2 extends this posture to destruction; it does not introduce it.
...
- **Node-side rejection plumbing** exists: `Prime`/`StartServing`/`StartStateful`/`StartGroupMember` already return `ResourceExhausted` on the max-live-VMs cap. PR 3 widens the predicate and teaches the dispatcher to retry.

---

## PR 1: `isolated_execution` workload contract (ADR decision 6)

Branch: `feat/embervm-isolated-execution`. Self-contained; no dependency on the consistency rework. Ships enforcement ON (no env gate needed: with the flag defaulting false and no workload setting it, behaviour is unchanged until a workload opts in).

### Task 1.1: CRD schema field

**Files:**
- Modify: `projects/embervm/chart/crds/workload-crd.yaml` (spec properties block)

**Steps:**
1. Add to the Workload CR `spec` schema, alongside the existing workload class fields:
   ```yaml
   isolatedExecution:
     type: boolean
     default: false
     description: >-
       Platform-enforced single-use contract (ADR embervm/014 decision 6).
       When true the instance is booted fresh for this workload and is never
       returned to a warm pool, relit, resumed, banked, or snapshotted; its
       slot, scratch volume, and network identity are released only after
       node-confirmed destruction.
   ```
2. Note the large-CRD server-side-apply gotcha: this CRD is applied with SSA (see `reference_argocd_large_crd_serversideapply`); a field addition is safe, no annotation-size risk.
3. Commit: `feat(embervm): add isolatedExecution to Workload CRD schema`

### Task 1.2: Catalog parse + accessor

**Files:**
- Modify: `projects/embervm/control/lib/embervm/workload_watcher.ex` (CR -> entry mapping)
- Modify: `projects/embervm/control/lib/embervm/workload_catalog.ex` (entry map + accessor)
- Test: `projects/embervm/control/test/embervm/workload_catalog_test.exs`

**Steps:**
1. Write failing ExUnit tests: `WorkloadCatalog.isolated_execution?("w")` returns `true` when the CR spec carries `isolatedExecution: true`, `false` when absent (default), `false` when explicitly false.
2. In `workload_watcher.ex`, map `spec["isolatedExecution"]` (default `false`) into the catalog entry at upsert.
3. In `workload_catalog.ex`, add `isolated_execution?/1` reading the ETS entry, defaulting `false` for unknown workloads (fail-open here is correct: an unknown workload cannot have opted in, and dispatch gates on catalog presence elsewhere).
4. Commit: `feat(embervm): parse isolatedExecution into WorkloadCatalog`

### Task 1.3: CP refusal gates on every reuse transition

**Files:**
- Modify: `projects/embervm/control/lib/embervm/session_manager.ex` (bank path)
- Modify: `projects/embervm/control/lib/embervm/serving_sweeper.ex` and `serving_manager.ex` (idle-bank path)
- Modify: `projects/embervm/control/lib/embervm/stateful_sweeper.ex` and `stateful_manager.ex` (idle-bank path)
- Modify: `projects/embervm/control/lib/embervm/group_manager.ex` (`bank_group/1`)
- Modify: `projects/embervm/control/lib/embervm/pool_manager.ex` (refill: never Prime a warm pool for a flagged workload)
- Modify: `projects/embervm/control/lib/embervm/dispatcher.ex` (warm-tier claim: flagged workloads always take the MISS path, fresh boot)
- Test: each manager/sweeper test file listed in the repo test layout

**Steps (repeat write-failing-test -> implement -> commit per surface, one commit per surface):**
1. Add a single helper, e.g. `Embervm.IsolatedExecution.refuse_reuse(workload, transition)` returning `{:error, :isolated_execution}` when flagged, so every gate reads identically and the semgrep/grep surface is one symbol.
2. Gate order, matching where each transition is decided (before any RPC is issued):
   - SessionManager.bank, ServingSweeper idle-bank, StatefulSweeper idle-bank, GroupManager.bank_group: refuse bank; the instance instead follows its lifetime-expiry destroy path.
   - Relight/wake entry points in the same managers: refuse relight (a flagged instance is never banked, so a relight request for one indicates drift; destroy it, decision 6 "found in any state other than running or destroying is a defect").
   - PoolManager.refill: skip flagged workloads entirely (no primed inventory).
   - Dispatcher warm tier: treat flagged workloads as MISS-only (fresh Prime+Assign per invocation).
   - EvictSnapshot/snapshot-capture entry points: refuse capture for flagged instances.
3. Reconciler defect rule: in each adoption reconcile, a flagged instance discovered banked/snapshotted is evicted and destroyed, with an oplog op recording why (`:isolated_execution_defect` reason field on the existing destroy op, not a new op kind, to keep the TLA vocabulary delta small).
4. Tests per surface: flagged workload's session reaches idle timer -> destroy op appended, no `:session_banked` op, no Bank RPC issued (assert via stub `channel_fun`).
5. Commits: `feat(embervm): refuse pool/bank/relight/snapshot for isolated_execution workloads` (split per surface as convenient).

### Task 1.4: TLA vocabulary classification + chart bump + CI

**Files:**
- Modify: `projects/embervm/specs/vocabulary.exs` (classify any new reason strings; no new verbs or op kinds expected)
- Chart bump: `bazel/tools/git/bump-chart.sh projects/embervm`

**Steps:**
1. Run the vocabulary layer-1 test expectations mentally: no new proto verbs, no new op kinds (reason field only), so only confirm the guard passes in CI. If the guard flags anything, classify it as modeled-excluded with a one-line justification referencing ADR 014 decision 6.
2. `bazel/tools/git/bump-chart.sh projects/embervm` (CRD + CP behaviour change must deploy).
3. `bazel/tools/format/fast-format.sh`, commit, push, open PR, `gh pr checks --watch`. Fix CI via `mcp__buildbuddy__get_invocation` -> `get_log`, quoting failures verbatim.

---

## PR 2: Node-confirmed destruction (ADR decision 5)
...
### Task 2.1: Proto + node agent: Destroy confirms teardown
...
### Task 2.2: CP `destroying` state + synchronous destroyed record
...
### Task 2.3: Fail-closed reconciliation in both directions
...
2. Direction 2 (node reports, CP unaware): an instance a node reports that no CP record matches is an orphan to destroy, not adopt. Route through the node-confirmed destroy path from Task 2.2. Exclusions: primed-pool VMs (Dispatcher inventory, not store-backed) and instances younger than the grace window (async-write race, see PR 4, where the reconciler must adopt-and-backfill instead; the adopt-vs-destroy discriminator is "does a pending async write or task/session record reference this vm_id").
...
### Task 2.4: Vocabulary + spec + rollout
...
## PR 3: Reject/retry placement (ADR decision 3)
...
### Task 3.1: Node-side cheap rejection under real pressure
...
2. Rejection must be cheap: predicates read already-maintained counters (cgroup budget, allocator freelist); no disk or netlink work before rejecting.
...
### Task 3.2: CP dispatcher retry-next-brick
...
## PR 4: Async durable writes off the boot/wake hot path (ADR decision 2)

Branch: `feat/embervm-async-lifecycle-writes`. Gate: `EMBERVM_ASYNC_LIFECYCLE_WRITES` (default off). Depends on PR 2 (the adopt-and-backfill reconciler is the repair path) and PR 3 is recommended first (retry makes the write-lost window smaller in practice).

Scope: the `:assigned`/`:started` appends on the task dispatch path and `:session_created`/`:session_relit` on the session boot/wake path move behind the instance becoming interactive. Explicitly NOT async: `:submitted` (quota audit trail), all metering ops, all destruction ops (PR 2), all bank ops (bank is not a hot path and its snapshot identity must be durable before eviction).

### Task 4.1: Async append worker with ordered per-instance queue
...
### Task 4.2: Rewire dispatch and session-boot ordering behind the gate
...
### Task 4.3: Vocabulary, bump, rollout

**Steps:**
1. No new op kinds expected; classify anything the guard flags.
2. Chart bump, push, PR, CI. Flip `EMBERVM_ASYNC_LIFECYCLE_WRITES=1` only after PR 2's gate has soaked clean, and watch the adoption-backfill counter for a week (a nonzero steady rate is expected and fine; a growing rate means the writer is losing races it should not).

---

## PR 5: Tap pre-provisioning at brick boot (ADR decision 4)

Branch: `feat/embervm-tap-preprovision`. Node-agent-only; gate via noded flag `--tap-prealloc` (default 0 = off).

### Task 5.1: Tap pool in the node agent
...
2. `AllocateTap` draws a pre-created tap (bring link up, wire DNAT) instead of creating one; falls back to on-demand creation when the pool is empty (pool exhaustion also feeds PR 3's `pressure:taps` rejection). `ReleaseTap` returns the tap to the pool (link down, DNAT removed) instead of deleting, when prealloc is on.
...
### Task 5.2: Wire the flag, bump, rollout
...
## PR 6: TLA+ follow-through (mandatory per ADR 014)

Branch: `docs/embervm-adr014-tla-followthrough`. Runs AFTER PR 2 and PR 4 land (ADR 006 rule: specs are not updated while the protocol churns).

### Task 6.1: Re-check `adoption.tla` against worker authority
...
2. Add the destroying-state transition from PR 2 with the new invariant: **no instance is recorded destroyed before its owning node confirms teardown.**
3. Commit: `docs(embervm): adoption.tla reflects worker-authoritative semantics + destroy invariant`

### Task 6.2: Bank/relight + generation-pairing spec (ADR 006 protocol 2)
...
**Steps:**
1. Model against post-014 semantics with the four invariants ADR 014 names checkable: destroyed-only-after-node-confirm, `isolated_execution` instance never reaches pool return/relight/snapshot, no wake resumes a stale snapshot, stored volume generations never regress (fold in the pair-broken monotonic-floor fix from PR #3770).
...
| -- | ---- | ---------- | ------------- |
| 1 isolated_execution | none (flag opt-in is the gate) | none | n/a; first flagged workload is the canary |
| 2 node-confirmed destroy | `EMBERVM_NODE_CONFIRMED_DESTROY` | none | 1 day soak, destroying-alarm silent |
| 3 reject/retry | `EMBERVM_PLACEMENT_RETRY` (CP half) | none (better after 2) | rejection reasons visible in logs, no retry storms |
| 4 async writes | `EMBERVM_ASYNC_LIFECYCLE_WRITES` | 2 (repair path), 3 recommended | adoption-backfill counter flat over a week |
| 5 tap prealloc | `--tap-prealloc` noded flag | none | canary brick class, then fleet |
| 6 TLA follow-through | n/a (docs/specs) | 2 and 4 merged | TLC green in CI, negatives still fail |
