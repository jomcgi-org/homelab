...
def request_path() -> Figure:
    """Six stages of one request, the same five parts held in place on each."""
    f = Figure(700, 948, "Follow one request through the tiers")
    slots = [(0, 0), (350, 0), (0, 316), (350, 316), (0, 632), (350, 632)]
    f.grid(700, 948, cols=[350], rows=[316, 632])
    titles = [
        "Make room",
        "Prepare the inputs",
        "Run the layer and route",
        "Get the selected experts ready",
        "Compute and join",
        "Emit and repeat",
    ]
    for i, ((px, py), title) in enumerate(zip(slots, titles)):
    ...
    # 1 Make room: KV grows inside A; the pin budget and C share the host.
    px, py = slots[0]
    f.arrow(px + 200, py + 94, px + 250, py + 94)
    f.arrow(px + 265, py + 138, px + 265, py + 189, both=True)
    f.lines(
        px + 54,
        py + 278,
        [
            "admit; reuse a matching prefix; allocate KV",
            "KV and expert slots share A: the pool starts",
            "small and grows; pin budget shares B with C",
        ],
    )

    # 2 Prepare the inputs: table rows read once into pinned staging, then A.
    px, py = slots[1]
    f.path_arrow(
        [
            (px + 220, py + 220),
    ...
    f.text(px + 272, py + 100, "io_uring")
    f.lines(
        px + 54,
        py + 278,
        [
            "n-gram ids follow from the tokens, so the",
            "rows are known before the forward: read once",
            "from D to pinned staging, then copied to A",
        ],
    )

    # 3 Run the layer and route: dense work on A, then the router picks.
    px, py = slots[2]
    f.text(px + 54, py + 112, "attention or GDN on A, then the router")
    f.text(px + 54, py + 282, "hidden state")
    f.arrow(px + 136, py + 278, px + 150, py + 278)
    f.box(px + 152, py + 268, 46, 20)
    f.text(px + 158, py + 282, "router")
    ...
    for i in range(24):
        ...
        h = 18 if i in tall else 8
        f.line(tx, py + 290, tx, py + 290 - h, weight=1.25 if i in tall else 1)
    f.text(px + 232, py + 304, "512 each, top-10")
    f.text(px + 54, py + 304, "prefill: many; decode: few")

    # 4 Get the selected experts ready: three residencies, three paths.
    px, py = slots[3]
    f.arrow(px + 90, py + 118, px + 90, py + 102)
    f.text(px + 96, py + 114, "PCIe")
    f.text(px + 140, py + 114, "hot: stays")
    ...
    f.lines(
        ...
        py + 278,
        [
            "hot: already on A, no transfer",
            "pinned: copied from B over PCIe into a slot",
            "cold: read from D through C, computed on E",
        ],
    )

    # 5 Compute and join: A and E run at once; the layer waits for the slower.
    px, py = slots[4]
    f.callout(px + 300, py + 88, "+")
    f.arrow(px + 258, py + 80, px + 291, py + 86)
    f.arrow(px + 300, py + 118, px + 300, py + 97)
    f.text(px + 312, py + 92, "out")
    f.text(px + 54, py + 280, "A")
    f.box(px + 70, py + 272, 80, 8)
    f.text(px + 54, py + 296, "E")
    f.box(px + 70, py + 288, 190, 8)
    f.line(px + 260, py + 268, px + 260, py + 298, dashed=True)
    f.text(px + 266, py + 298, "join")
    f.text(px + 54, py + 312, "the next layer waits for the slower side")

    # 6 Emit and repeat: head, sample, stream; counters retune the hot set.
    px, py = slots[5]
    f.box(px + 54, py + 272, 40, 20)
    f.text(px + 60, py + 286, "head")
    f.arrow(px + 94, py + 282, px + 104, py + 282)
    f.box(px + 106, py + 272, 52, 20)
    f.text(px + 112, py + 286, "sample")
    f.arrow(px + 158, py + 282, px + 168, py + 282)
    f.cells(px + 170, py + 278, 8, 8, 1)
    f.text(px + 184, py + 286, "token")
    f.text(px + 54, py + 306, "streamed, then back to 2")
    heights = [26, 22, 18, 15, 12, 10, 8, 7, 6, 5, 4, 3]
    for i, h in enumerate(heights):
        bx = px + 232 + i * 7
        f.line(bx, py + 296, bx, py + 296 - h, weight=1.25)
    f.line(px + 228, py + 296, px + 316, py + 296)
    f.text(px + 228, py + 310, "hot-set counters")
    return f
...
def _machine_parts(
    ...
    *,
    ...
    """The five tier parts of one machine, in the same place on both panels."""
    x = lambda v: px + v  # noqa: E731
    y = lambda v: py + v  # noqa: E731
    f.box(x(48), y(40), 210, 62, tone="gpu")
    f.text(x(54), y(54), gpu, tone="gpu")
    ...
    f.lines(x(278), y(134), ["CPU", "exec"])
    f.box(x(48), y(118), 210, 40, tone="ram")
    f.lines(x(54), y(134), ram, tone="ram")
    f.box(x(48), y(172), 210, 34, dashed=True, tone="cache")
    f.text(x(54), y(192), "page cache", tone="cache")
    f.box(x(48), y(220), 210, 40, tone="disk")
    f.lines(x(54), y(236), disk, tone="disk")
    if letters:
        f.keyed(x(26), y(71), "A", x(48), y(71), tone="gpu")
        f.keyed(x(26), y(138), "B", x(48), y(138), tone="ram")
        f.keyed(x(26), y(189), "C", x(48), y(189), tone="cache")
        f.keyed(x(26), y(240), "D", x(48), y(240), tone="disk")
        f.keyed(x(318), y(174), "E", x(318), y(158))


def two_machines() -> Figure:
    """The same tiering, two different cold-expert paths."""
    f = Figure(700, 316, "Choose the path that finishes sooner")
    slots = [(0, 0), (350, 0)]
    f.grid(700, 316, cols=[350], rows=[])
    f.panel(0, 0, 1, "Qwen on the RTX 4090")
    f.panel(350, 0, 2, "GLM on the RTX PRO 6000")

    # 1 Qwen: nothing pins whole, so the cold tail computes on E and only an
    ...
    _machine_parts(
        ...
        py,
        gpu="GPU: 24 GB VRAM",
        ram=["pinned banks, 40 GB", "of 64 GB host RAM"],
        disk=["expert banks, 63.5 GB", "table, 27 GB"],
        letters=True,
    )
    f.arrow(px + 70, py + 220, px + 70, py + 206, dashed=True)
    f.text(px + 78, py + 216, "willneed")
    f.path_arrow([(px + 258, py + 189), (px + 286, py + 189), (px + 286, py + 158)])
    f.text(px + 262, py + 203, "read")
    f.path_arrow([(px + 300, py + 118), (px + 300, py + 71), (px + 258, py + 71)])
    f.text(px + 306, py + 100, "kB out")
    f.lines(
        px + 54,
        py + 278,
        [
            "cold tail: computed on E, kilobytes back",
            "over PCIe instead: 10 vs 19.5 tok/s",
        ],
    )

    # 2 GLM: most of the model pins, so a miss streams weights over PCIe and
    # the CPU tail loses.
    px, py = slots[1]
    _machine_parts(
        f,
        ...
        py,
        gpu="GPU: 96 GB VRAM",
        ram=["pinned, 112 GB: 29 layers", "of 176 GB host RAM"],
        disk=["expert banks, 13 layers", "on local NVMe"],
        letters=False,
    )
    f.arrow(px + 90, py + 118, px + 90, py + 102)
    f.arrow(px + 108, py + 118, px + 108, py + 102)
    f.text(px + 116, py + 114, "PCIe: about 3.1 GB a token")
    f.arrow(px + 70, py + 220, px + 70, py + 206, dashed=True)
    f.text(px + 78, py + 216, "hot set covers 99.8%")
    f.lines(
        px + 54,
        py + 278,
        [
            "a miss streams a 12 MB expert to A",
            "tail on E instead: 9.8 vs 12.1 tok/s",
        ],
    )
    return f
